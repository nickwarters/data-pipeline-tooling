...
- **Layout:** `framework/` (reusable engine, organised into the four public
  ...
  observability / `targeted_selection` utilities in the top-level `tools/` package — both siblings of
  `framework/`,
...
- **Public API:** application code (`pipelines/` + the `case_review/`
  ...
  `tools.migrations` / `tools.deliverables` /
  `tools.targeted_selection`) are a sibling
...
the resolved base directory up to date — or just those named by a repeatable
`--subject <subject>` (every database under it) or
`--database <subject>/<database>`, a name the tree does not carry being an
error — with `--check` reporting what is pending and exiting non-zero
