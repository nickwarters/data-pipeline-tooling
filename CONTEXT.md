...
**Pass Rule**:
A named, per-Case-Type declaration of which of a **Question Bank**'s **Outcome Options** count as a *pass* and which as a *fail*, for **Reporting**. Declared in code in `case_review/pass_rules.py` (`PassRule`; `standard` and `strict` today) because which outcomes pass is a case-review reporting judgement, not framework and not pipeline mechanics; the `answer_pass_rate_current` **Aggregate table** on the `cora_platform_metric` subject carries the rule's name in its grain, so one table serves reports that draw the line differently. A rule must classify *every* outcome id the current bank declares for a Case Type it covers, exactly once — checked against the bank on every run, so a bank that grows an outcome fails the run naming the rule, the Case Type and the id rather than landing a silent pass or fail; and an Answer on a Case Type no rule covers fails it too, since "not declared" would otherwise look like "considered and declined". **Distinct from the review platform's `is_failure`** (`platform_frontend/docs/reporting-data-contract.md`), which calls every non-default outcome a fail — so "Good with process enhancement" is a failure there and a pass under `standard`. The platform keeps its own rule for its own UI; nothing under `platform_frontend/` is consulted for the line or changed by it, and a bank's `defaultOutcomeId` is never edited to make the two agree.
_Avoid_: failure test, `is_failure` (the review platform's rule), threshold, scoring
