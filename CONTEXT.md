...
The `reviewer_activity` Report Feed is refreshed Monday-Friday with the Sync
freshness check. Known stale Sync history blocks normal publication, while no
successful Sync history is allowed with a warning under the current first-run
policy. After Friday, the live tail may use an artifact complete through
Thursday until the next scheduled working-day run succeeds.
