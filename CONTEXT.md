...
| **Sync** | platform-wide: **one** subject for every Case Type, one set of tables, discriminated by `case_type` (intended as `cora_cases`; the current walking skeleton writes `sharepoint_cases`) | **History-upstream / current-gold**, like Ingest: raw + silver accumulate every **Version observation**; gold is `Refresh()`-rebuilt and current-only — one row per Case, plus **Detail Tables** reduced by **observation-snapshot reduction**. ([ADR-0016](docs/adr/0016-one-sync-subject-for-every-case-type.md)) |
