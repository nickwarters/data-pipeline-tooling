...
**Targeted selection**:
A narrowing technique Selection may apply (`tools.targeted_selection`): reduce a pool to a declared count per **cell** — one combination of values across a declared, coarsest-first list of grouping columns (e.g. brand → case type → product → attribute) — under an overall **total** that caps the result. A cell short of Cases is **made up** from the next level up (same brand/case type/product, any attribute), climbing no higher than a declared **make-up group** (e.g. brand/case type); every cell takes its own Cases before any make-up, so a short cell never takes Cases a sibling was targeted at. An optional **balance** steers one Boolean column towards 50/50 within the groups of a declared prefix of the levels — a preference, never a gate. Reproducible like **Sampling**: a pure function of input + seed.
_Avoid_: quota (reserved for **Variation quota**), split (reserved for a **Selection group**'s volume proportions), backfill (a **Void replacement** avoid-term)
...
**Question Bank**:
The set of questions a Reviewer works through when reviewing a Case; the attribute that most often distinguishes one Variation from another. Its **content** is the review platform's outright (see *Question Bank ownership* below), and this half of the system only reads it — as the **published export artifact** the platform specifies in [`platform_frontend/docs/reporting-data-contract.md`](platform_frontend/docs/reporting-data-contract.md): JSON in a `.txt` file (SharePoint SE is unreliable serving `.json` — the same constraint a **Report Feed** carries), one file per Case Type for the *current* bank and one per published *version* beside it. Reading it goes through the Shared Reader `readers/question_banks.py`, whose `QuestionBankStore(base_dir)` mints two Readers, one per grain: `qb_reader(...)` for a bank's questions and `outcomes_reader(...)` for its **Outcome Options** (below). Every argument narrows — naming no Case Type stacks *every* Case Type into one dataset for the cross-Case-Type read, and `current=False` reads the *published snapshots* rather than the mutable heads, which is the cross-time read a report spanning Cases completed against different banks joins against. The heads (`{slug}.txt`) and the snapshots (`{slug}.{version}.txt`) are separate reads on purpose: a head declares the version it was last published as, so the same bank sits under two names and reading both would double every figure grouped by version. Each lands one row per question or per option, with the artifact's envelope — `slug`, `version`, `default_outcome_id` — stamped on every row. **Which** version to read is the Case's answer, not the reader's: a completed Case carries the identifier it was reviewed against in `questionBankVersion`, and reporting against anything else is reporting a Reviewer's work against questions they never saw. Note the two glossaries do *not* line up here — the platform's **Question Bank** entry is the authoring form and its *catalogue* the runtime one, while what crosses to this side is a third thing, the compiled reporting export.
_Avoid_: questionnaire, form, checklist (until disambiguated), catalogue (the review platform's runtime term for the same content — see [`platform_frontend/CONTEXT.md`](platform_frontend/CONTEXT.md))

**Outcome Option**:
One of the verdicts a **Question Bank** declares its questions' answers may map onto — an `id`, a `wording` (what a Reviewer picks), and a **`severity`**, the score that ranks them. Severity is what makes an Outcome comparable at all: the review platform resolves a Case's verdict as the highest-severity applicable mapped outcome, and the bank's `default_outcome_id` names the one that is not a failure — an answer mapped to anything else is a failed Answer. Declared once per Case Type per published version, so *whether two Case Types score the same wording the same way* is a real question, and `readers/question_banks.py`'s argument-less `outcomes_reader()` is what answers it. **The review platform's glossary does not define this term**; the name is taken from the export artifact's own `outcomeOptions` field, and the platform's **Outcome** entry means the computed *verdict*, not the option it was drawn from — so say which you mean when a change spans both halves.
_Avoid_: outcome (unqualified — the review platform's **Outcome** is the computed verdict), score, rating, severity (that is the option's field, not the option)
...
so the Forwarder is a small application with one declared **task** per kind of
delivery, not a file-copying script. _Here_: its own top-level project,
`forwarder/`, with its own `CONTEXT.md` and `docs/adr/`; it shares exactly one
thing with the pipeline framework — the outbox layout `tools.deliverables`
owns — and imports nothing from it. *(The loop, its declared tasks and its retry
handling now exist; both SharePoint clients are stubs that raise, dead-lettering
is unimplemented, and it reads a `QUEUE/` directory beside its own package
rather than the deliverable outbox. Its own glossary in
[`forwarder/CONTEXT.md`](forwarder/CONTEXT.md) is authoritative for its terms.)*
...
(`tools/source_checkpoint.py`) keeps one **watermark** per polled source — keyed
by a stable string key, so a SharePoint list is its GUID and a table its
qualified name, never a display name — under
`<base_dir>/_checkpoints/sources.db`, the one store for every such source
([ADR-0030](docs/adr/0030-source-checkpoints-are-one-generic-store.md)). The
rule turning a watermark into the next span to fetch is the pure
`instant_window` beside it — `end = source_now - safety_lag`, `start =
committed - overlap` (`None` on a first load, meaning everything the source
holds), and no window at all when `end` has not passed `committed` — handed the
watermark rather than reading it, because overlaps and lags belong to the
source being polled. The commit is
the **last act of a successful run**; nothing else advances it, and **a run's
time is never a source's watermark** — the run registry's "last success" is a
