...
**Targeted selection**:
A narrowing technique Selection may apply (`case_review.targeted_selection`): reduce a pool to a declared count per **cell** — one combination of values across a declared, coarsest-first list of grouping columns (e.g. brand → case type → product → attribute) — under an overall **total** that caps the result. A cell short of Cases is **made up** from the next level up (same brand/case type/product, any attribute), climbing no higher than a declared **make-up group** (e.g. brand/case type); every cell takes its own Cases before any make-up, so a short cell never takes Cases a sibling was targeted at. An optional **balance** steers one Boolean column towards 50/50 within the groups of a declared prefix of the levels — a preference, never a gate. Reproducible like **Sampling**: a pure function of input + seed.
