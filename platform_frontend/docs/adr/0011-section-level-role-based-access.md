...
- The Case page (`src/cora-case-review.js`) computes the viewer's role(s) once on mount from the loaded Case row + the user's group membership (already resolved in `src/permissions.js`), then calls the evaluator per section.
