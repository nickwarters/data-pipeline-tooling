...
- [ ] **On every existing `Cases-{slug}` list, in both prod and UAT**: add
      `To-allocate` to the `Status` choice column. Without it, ingestion cannot
      write a Case at all — SharePoint rejects a value the column does not
...
| `AssignedAt`                                            | Date and Time                                                                         |  **✓**  | Required when a Reviewer is assigned; client-written with `AssignedReviewer`. The Action Centre ages and orders “N days in progress” from this current-allocation clock and never falls back to `Created`.                                                                                                                                                      |
...
| `OnHold`                                                | Yes/No                                                                                |  **✓**  | Reviewer hold state and Action Centre reason flag; indexed for allocation capacity counts and the On Hold group's `$count`.                                                                                                                                                                                                                                     |
