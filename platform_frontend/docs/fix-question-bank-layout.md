...
Alternatively, use `[data-cora-root]:has(cora-bank-editor)` — works in Edge Chromium, which is the browser baseline — and avoid touching `app.js`. Pick this if you prefer a pure-CSS fix; it's the smaller change.
...
- Don't change the `cora-` prefix or component scoping rules.
