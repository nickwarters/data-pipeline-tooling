...
**`In-progress`** → (**Send Actions** if any Remediation Actions exist) **`Actions In
Progress`** → **`Completed`**, or `In-progress` → (**Complete Case** when no actions) →
`Completed`. The completion control lives on the **Summary** tab and drives the
transition, labelled **"Send Actions"** when actions exist and **"Complete Case"**
otherwise. A third, terminal exit leaves either live status: **`Void`**
(see **Void Case**), reached from the two-step Void control on **Case Details** and never
returned from.
_Avoid_: State (overloaded), phase

**`To-allocate`**:
The status a Case is created in, and the one the **Hopper** is made of: ingested and
waiting for a Reviewer to claim it. It is what **allocation** reads — the pot is a value
on the indexed `Status` column, never "an `In-progress` row with an empty **Assigned
Reviewer**", so a Case waiting for anyone is one indexed predicate rather than a scan of
every live Case in the list. Nothing is stamped on it yet: no Assigned Reviewer, and no
`DueDate` — the review SLA is stamped by the claim, so no review clock runs against an
unclaimed Case and it can never read as overdue. The claim moves it to `In-progress` in
the **same PATCH** as the Reviewer, under one `If-Match`, so no row can hold a Reviewer
while still advertising itself as unclaimed. Nothing returns a Case to it: an
`In-progress` Case handed to a different Reviewer is _reassigned_, which leaves the
status alone.
_Avoid_: Pending, Unassigned (the state of the person field, not the lifecycle),
New
...
**Case Details**:
The **Section** that displays the **Case Type**-specific descriptive fields that frame a Case — e.g. customer name, account numbers, relevant dates. The set of fields is declared per **Case Type**, so different types show different details. Read-only for every role and never editable. Its tab is visible to the reviewing, owning and **Controls** roles and is the default view on the case review page; the **Responsible Party** and their **Manager** have no Details tab and read the same fields as the first block of the **Summary** instead (the block follows the Summary's access, not the hidden tab's). One of the Sections alongside Questions, Conversation, Notes, Remediation and Summary.
...
**General Question**:
A **Case Type**-configured field a **Reviewer** answers on the Review tab above or beneath every **Applicable Question** (`generalQuestionsPlacement`, default beneath), behind a rule and its own section title (`generalQuestions` in the Case Type module, using a subset of the **Issue Capture Field** type vocabulary: `text`, `textarea`, `select`, `radio` — the subset whose answer is a plain string, since `person` needs per-field people-search state the Review tab does not carry). A Case Type builds its list with `resolveGeneralQuestions()`, including **shared General Questions** by key from `case-types/general-questions.js` and declaring anything Case Type-specific inline beside them; a shared question's key is fixed by the catalogue, so the same question means the same answer key in every Case Type and cannot be reworded per Case Type. The one thing an inclusion may add is `required` (`{ key: 'reviewChannel', required: true }` in place of the bare key), because how hard a Case Type presses for an answer is not part of what the question asks; an inclusion setting anything else is refused at load time, and the two object forms are told apart by `type`, which an inline question always carries and an inclusion never does. Deliberately _not_ outcome-driving: General Questions carry no `showWhen` and no failure criteria, and their answers — namespaced `general:<key>` in the Case's **Answer** blob — reach no catalogue-driven evaluator, so applicability, **Question Group** progress and the **Outcome** are all unaffected. They do gate the _timing_ of the transition out of `In-progress`: a field declaring `required: true` — the same key an **Issue Capture Field** carries, meaning the same thing — holds the **Summary** tab's **Send Actions** / **Complete Case** control disabled, with its reason, until it is answered. It is one of four pre-send content gates, alongside the "Is remediation required?" decision, the required Issue Capture Fields and the **Responsible Party**, and is named first of the four because the Review tab comes before the Issues tab. The gate walks the **Case Type**'s declared field list rather than the catalogue, which is what keeps it a question about _when_ the **Reviewer** may send and never about what the Case concludes; a Case Type marking nothing `required` is gated exactly as it was before the key existed, and the final close of an `Actions In Progress` Case never asks again, since nothing could have got past the send unanswered. ([ADR-0050](./docs/adr/0050-required-general-questions-gate-the-send-not-the-outcome.md)) Answered General Questions also roll up read-only on the Summary tab (same `generalQuestionsPlacement` side, unanswered ones omitted), so the **Case Type Owner** reads what the Reviewer wrote. Both tabs render the _configured_ fields, so removing a General Question from a **Case Type** hides answers already given to it: the `general:<key>` entries stay in the **Answer** blob (the pruning loop leaves non-catalogue keys alone) but appear nowhere in the UI — restoring the field with the same key brings them back. Never a **Question Definition**: General Questions do not live in the **Question Bank** and are not editable in the bank editor.
