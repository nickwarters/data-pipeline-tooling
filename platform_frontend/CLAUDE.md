...
3. **[docs/adr/](./docs/adr/)** — 55 architecture decisions, numbered
   (`0001`–`0055`). Read the status before relying on an older decision, and do
...
- **Case Review Sections declare themselves (ADR-0053), and the application
  ...
  **Adding a Section recipe:** author the plugin module, then add one import and
  ...
  composition root is irreducible, and it is the same file that names a page. Enabling it for a Case Type is that Case Type's
...
src/
  ...
  components/                   # reusable pure views, layered by dependency
    ...
    sections/                   # domain-feature units: take config, wire base components together
      ...
      cora-app-nav.js           # the nav bar as a pure view: draws the items chrome hands it and
                                #   reads no capability. Which links a user gets is derived from
                                #   APP_CONFIG.pagePlugins by navItemsFor() (ADR-0055)
  ...
  setup/                        # app startup helpers
    ...
    register-routes.js            # the page ENGINE, and it does more than the name says: routeTable()
                                  #   binds each APP_CONFIG.pagePlugins entry to the boot context (a
                                  #   guard becomes a no-arg closure that bounces, a loadOverride
                                  #   resolves the host's substitute loader) and registerRoutes()
                                  #   registers it, catching per entry; navItemsFor() derives the nav
                                  #   bar and resolveDefaultLandingPath() where a user lands, ranked
                                  #   by defaultForOrder with #/ the named fallback. Names no page
