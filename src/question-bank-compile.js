...
export function compileBank(bank) {
  const lines = [];
  lines.push(`// @ts-check`);
  lines.push(`/** @typedef {import('../src/sharepoint-client.js').CaseTypeConfig} CaseTypeConfig */`);
  lines.push(`/** @typedef {import('../src/sharepoint-client.js').Answer} Answer */`);
  lines.push('');
  ...
  for (const q of bank.questions) {
    ...
    if (q.category) lines.push(`      category: ${JSON.stringify(q.category)},`);
    lines.push(`      responseType: ${JSON.stringify(q.responseType)},`);
    if (q.options) lines.push(`      options: ${JSON.stringify(q.options)},`);
    if (q.showWhen) lines.push(`      showWhen: ${JSON.stringify(q.showWhen)},`);
    if (q.failureCriteria) lines.push(`      failureCriteria: ${JSON.stringify(q.failureCriteria)},`);
    if (q.remediationActions) {
      lines.push(`      remediationActions: [`);
      for (const r of q.remediationActions) lines.push(`        ${JSON.stringify(r)},`);
      lines.push(`      ],`);
    }
    if (q.allowFreeFormRemediation) lines.push(`      allowFreeFormRemediation: true,`);
  ...
  lines.push(`    const hasNo = Object.values(answers).some(a => a.value === 'No');`);
...
export function highlight(code) {
  return escapeHtml(code)
    ...
    .replace(/\b(const|export|default|function|return|some|of)\b/g, '<span class="k">$1</span>')
