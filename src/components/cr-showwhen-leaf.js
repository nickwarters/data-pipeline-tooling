...
export class CRShowwhenLeaf extends ReactiveElement {
  ...
  render() {
    ...
    return h('div', { class: 'leaf' },
      h('select', {
        onchange: (/** @type {any} */ e) => commit(() => { leaf.qId = e.target.value; commitTreeFor(q); })
      }, ...others.map((/** @type {any} */ o) => 
        h('option', { value: o.id, selected: o.id === leaf.qId }, o.id + (o.deprecated ? ' (deprecated)' : ''))
      )),
      h('select', {
        class: 'leaf-op',
        onchange: (/** @type {any} */ e) => commit(() => {
      ...
      }, ...[['equals','='], ['in','is one of'], ['answered','has been answered']].map(([k, lbl]) => 
