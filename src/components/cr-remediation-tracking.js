 ...
 * responsibility: a status change is re-dispatched as a bubbling `cr-action-status`
...
export function RemediationTracking(props) {
  ...
  if (rows.length === 0) {
    return [
      ...
      h(
        'p',
        { class: 'cr-remediation-tracking-empty' },
        'No remediation actions sent.'
      ),
    ];
  }

  const list = h('ul', { class: 'cr-remediation-tracking-list' });
...
export function renderActionRow(props, row) {
  ...
  const li = h('li', { class: 'cr-remediation-tracking-item' });
  li.appendChild(h('p', { class: 'cr-tracking-question' }, question.text));
  li.appendChild(h('p', { class: 'cr-tracking-action' }, action.text));

  if (!props.canResolve) {
    li.appendChild(
      h('p', { class: 'cr-tracking-status' }, `Status: ${action.status}`)
    );
    if (action.status === 'cancelled' && action.cancelReason) {
      li.appendChild(
        h(
          'p',
          { class: 'cr-tracking-cancel-reason' },
  ...
  const reasonInput = /** @type {HTMLInputElement} */ (
    h('input', {
      class: 'cr-tracking-cancel-input',
      type: 'text',
  ...
  const select = /** @type {HTMLSelectElement} */ (
    h(
      'select',
      {
        class: 'cr-tracking-status-select',
        value: action.status,
        onchange: () => {
...
export class CRRemediationTracking extends ShellElement {
  constructor() {
    super();
  ...
  _dispatchStatus(questionId, fieldKey, actionId, status, cancelReason) {
    this.dispatchEvent(
      new CustomEvent('cr-action-status', {
...
customElements.define('cr-remediation-tracking', CRRemediationTracking);
