...
export class CRRemediationEditor extends ReactiveElement {
  ...
  render() {
    ...
    const freeRow = h('div', { class: 'rem-free-row' },
      h('div', { innerHTML:
      ...
      h('div', { class: 'toggle' + (q.allowFreeFormRemediation ? ' on' : ''),
        onclick: () => commit(() => { q.allowFreeFormRemediation = !q.allowFreeFormRemediation; }) }),
    );

    const wrap = h('div', { class: 'rem-block' },
      h('h4', { innerHTML: `Remediation Actions <span class="rem-count">(${count}${q.allowFreeFormRemediation ? ' + free-form' : ''})</span>` }),
      freeRow
    );

    if (q.allowFreeFormRemediation) {
      wrap.appendChild(h('div', { class: 'rem-free-preview', innerHTML:
    ...
    (q.remediationActions || []).forEach((/** @type {string} */ r, /** @type {number} */ idx) => {
      ...
      wrap.appendChild(h('div', { class: 'rem-item' },
        inp,
        h('span', { class: 'x', onclick: () => commit(() => {
    ...
    if (count === 0 && !q.allowFreeFormRemediation) {
      wrap.appendChild(h('div', { class: 'rem-empty' },
        '// no remediations — reviewers will see none on failure'));
    }

    wrap.appendChild(h('button', { class: 'tag-add rem-add',
