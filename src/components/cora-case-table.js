...
import { CORADataTable } from './cora-data-table.js';
import { caseRouteFor } from '../lib/case-route-links.js';

/** @typedef {import('../sharepoint-client.js').CaseRow} CaseRow */
/** @typedef {import('./cora-data-table.js').ColumnDef<CaseRow>} CaseColumn */
 ...
 * `cora-case-open`. Behaviour matches the pre-refactor table when no custom
 ...
 * @property {CORADataTable | null} inner
 * @property {(toolbar: HTMLElement) => void} onToolbar
 * @property {(inner: CORADataTable) => void} onInner
 ...
 * @returns {Array<HTMLElement | CORADataTable>}
 */
export function CaseTable({
  ...
  const innerTable = inner ?? new CORADataTable();
...
export class CORACaseTable extends ShellElement {
  constructor() {
    super();
  ...
  connectedCallback() {
    ...
    // <cora-data-table>. In a real browser that fires disconnected/connected on
  ...
  _openCase(row) {
    this.dispatchEvent(
      new CustomEvent('cora-case-open', {
...
customElements.define('cora-case-table', CORACaseTable);
