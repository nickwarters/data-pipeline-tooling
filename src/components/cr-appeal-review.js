...
export function AppealReviewSection(props) {
  ...
  if (props.access === 'edit' && open) {
    ...
    children.push(
      h(
        'p',
        { className: 'cr-appeal-review-empty' },
...
export function renderAppealSummary(appeal) {
  const children = [
    h('p', { className: 'cr-appeal-review-state' }, `State: ${appeal.state}`),
    h(
      'p',
      { className: 'cr-appeal-review-rationale' },
  ...
  if (appeal.resolution) {
    children.push(
      h(
        'p',
        { className: 'cr-appeal-review-resolution' },
        `Resolution: ${appeal.resolution.verdict} — ${appeal.resolution.rationale}`
      )
    );
  }
  return h('section', { className: 'cr-appeal-review-item' }, ...children);
}
...
export function renderResolveForm(props, appeal) {
  const agreeRadio = /** @type {HTMLInputElement} */ (
    /** @type {unknown} */ (
      h('input', {
        type: 'radio',
        name: 'cr-appeal-review-verdict',
        value: 'agreed',
        className: 'cr-appeal-review-verdict-agreed',
      })
    )
  );
  const rejectRadio = /** @type {HTMLInputElement} */ (
    /** @type {unknown} */ (
      h('input', {
        ...
        name: 'cr-appeal-review-verdict',
        value: 'rejected',
        className: 'cr-appeal-review-verdict-rejected',
      })
    )
  );

  const rationale = /** @type {HTMLTextAreaElement} */ (
    h('textarea', { className: 'cr-appeal-review-rationale-input' })
  );
  ...
  const outcomeSelect = /** @type {HTMLSelectElement} */ (
    h(
      'select',
      { className: 'cr-appeal-review-outcome-select' },
  ...
  const amendJustification = /** @type {HTMLTextAreaElement} */ (
    h('textarea', { className: 'cr-appeal-review-amend-justification' })
  );
  amendJustification.setAttribute('aria-label', 'Amendment justification');

  const error = h(
    'p',
    { className: 'cr-appeal-review-error', hidden: true },
    'A verdict and a rationale are required. If agreeing, an outcome and justification are also required.'
  );

  return h(
    'section',
    { className: 'cr-appeal-review-form' },
    h(
      'label',
      { className: 'cr-appeal-review-agree-label' },
      agreeRadio,
      h('span', {}, 'Agree')
    ),
    h(
      'label',
      { className: 'cr-appeal-review-reject-label' },
      rejectRadio,
      h('span', {}, 'Reject')
    ),
    ...
    h(
      'button',
      {
        className: 'cr-appeal-review-submit',
        onClick: () =>
          resolveAppeal(
            props,
            appeal,
...
export function resolveAppeal(
  ...
  const updatedAppeal = { ...appeal, state: /** @type {'resolved'} */ ('resolved'), resolution };
...
export class CRAppealReview extends ShellElement {
  constructor() {
    super();
    /** @type {CaseRow | null} */
    this.caseRow = null;
...
customElements.define('cr-appeal-review', CRAppealReview);
