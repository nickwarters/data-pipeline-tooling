...
This placeholder feed is called ``complaints_a``; the scaffold substitutes that token
(and its PascalCase class form ``ComplaintsA``) for the real feed name. It is a
*real, runnable* feed so its own ``test_complaints_a.py`` runs in the suite and keeps
the template honest.
"""
