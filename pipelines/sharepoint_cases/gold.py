...
``case_counts_current``      ``status`` x ``assigned_reviewer_manager_name``
...
from .schema import CASE_ID_NAMESPACE, FEED_NAME
...
def _case_counts(dataset: Dataset) -> Dataset:
    """Count current Cases by status and reviewer manager."""
    frame = dataset.to_pandas()
    manager = frame["assigned_reviewer_manager_name"].where(
        frame["assigned_reviewer_manager_name"].notna(), UNASSIGNED
    )
    counted = (
        frame.assign(assigned_reviewer_manager_name=manager)
        .groupby(["status", "assigned_reviewer_manager_name"])
        .size()
        .reset_index(name="case_count")
        .sort_values(["status", "assigned_reviewer_manager_name"], kind="stable")
    )
...
def case_counts_current_builder(
    ...
    **Grain: ``status`` x ``assigned_reviewer_manager_name``.**

    The second dimension is the Assigned Reviewer's manager, carried through
    under silver's own name. It is *not* a team: the provisioned list has no team
    column, and what the review platform calls "my team" is exactly the set of
    Cases whose ``AssignedReviewerManagerId`` is the signed-in user. Naming it
    ``owning_team`` would assert a dimension that does not exist, and shortening
    it to ``reviewer_manager_name`` would invent a synonym for a row that also
    carries ``responsible_party_manager_name``.

    A Case with no Assigned Reviewer manager is counted under the literal
    ``"(unassigned)"``. That is a reporting fill and never a source value; it
    exists because a NULL group key is a hole in the grain that a reader may
    silently drop, which would make the table quietly fail to add up to the
    ...
    return _aggregate_hop(
        ...
        step="count-by-status-and-manager",
        as_of=as_of,
        run_log=run_log,
    )
...
def case_age_buckets_current_builder(
    ...
    *,
    ...
    The reviewer-manager dimension is deliberately absent: it is one join away in
    ``case_current``, and carrying it here would multiply the table for a
...
def _aggregates() -> tuple[tuple[str, Callable[..., Pipeline]], ...]:
