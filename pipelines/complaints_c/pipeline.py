"""Case Type ingest for the ``complaints_c`` feed: source -> raw -> silver.
...
recorded under -- and so the label a downstream ``FreshnessRequirement`` names::
    ...
    python -m cli run pipelines/complaints_c --base-dir BASE_DIR
...
routes through the same ``run_pipeline``, so both record one identity::
    ...
    python -m pipelines.complaints_c.pipeline --base-dir BASE_DIR
"""
...
SOURCE_COLUMNS = ["record_id", "department", "resolution_days", "received_date"]
...
def to_silver(
    ...
    data = enforce(ComplaintsCRow, data, reject_writer=reject_writer, name="silver")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m pipelines.complaints_c.pipeline",
        description="Refine the complaints_c feed source -> raw -> silver.",
    )
    parser.add_argument(
        "--base-dir",
        dest="base_dir",
    ...
    # two, and complaint_selection's FreshnessRequirement("complaints_c") only ever
