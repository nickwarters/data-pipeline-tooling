...
def silver_builder(
    ...
    node = p.read(reader, name="read")
    node = p.transform(SchemaCoercion(ComplaintsCRow), node, name="coerce")
    if reject_writer is not None:
        node = p.quarantine(
            SchemaValueRulePartitioner(ComplaintsCRow),
            reject_writer,
            node,
            name="quarantine",
        )
    node = p.validate(SchemaValidator(ComplaintsCRow), node, name="post-validate")
    p.write(writer, node, name="write")
    return p
