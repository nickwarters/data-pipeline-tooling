...
This placeholder feed is called ``complaints_c``; the scaffold substitutes that token
(and its PascalCase class form ``ComplaintsC``) for the real feed name. It is a
*real, runnable* feed so its own ``test_complaints_c.py`` runs in the suite and keeps
the template honest.
"""
