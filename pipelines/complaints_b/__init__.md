...
This placeholder feed is called ``complaints_b``; the scaffold substitutes that token
(and its PascalCase class form ``ComplaintsB``) for the real feed name. It is a
*real, runnable* feed so its own ``test_complaints_b.py`` runs in the suite and keeps
the template honest.
"""
