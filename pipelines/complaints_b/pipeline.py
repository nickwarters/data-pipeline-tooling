...
def silver_builder(
    ...
    node = p.read(reader, name="read")
    node = p.transform(SchemaCoercion(ComplaintsBRow), node, name="coerce")
    if reject_writer is not None:
        node = p.quarantine(
            SchemaValueRulePartitioner(ComplaintsBRow),
            reject_writer,
            node,
            name="quarantine",
        )
    node = p.validate(SchemaValidator(ComplaintsBRow), node, name="post-validate")
    p.write(writer, node, name="write")
    return p
