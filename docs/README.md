...
| **Shared Reader** (`readers.<subject>`) | The supported way to read data your pipeline does **not** produce. A `Reader` over a named business dataset that crosses a subject boundary, declared once in `readers/<subject>.py` and constructed with a `base_dir` and nothing else (`CurrentCasesReader(base_dir=base_dir)`) — so a consumer names no layer, no table and no file, and renaming one changes one place. Read-only by construction (it mints no Writers) and a strict pass-through: no projection, coercion, join, aggregation or filtering. A pipeline still resolves the subject it *writes* through the medallion; that asymmetry is the design. Where a dataset is a **family** rather than a name — a **Question Bank**, two-dimensional in Case Type and version — the entry is a **store** instead (`readers/question_banks.py`): four methods, prefixed by grain — `qb_reader(...)` / `outcomes_reader(...)` for the questions and the **Outcome Options**, plus `qb_versions_reader()` / `outcomes_versions_reader()`. Naming no Case Type asks for every current bank; `current` and `version` name the same thing (the mutable head vs an immutable snapshot) so each is refused without the other, which puts the in-progress-vs-completed Case branch at the call site rather than inside the reader. The heads and the snapshots stay separate reads, because a head declares the version it was last published as and reading both double counts it. Still construction-time parametrisation, because the store is the factory. → [shared-readers.md](shared-readers.md) ([Shared Readers declare a cross-subject read once](adr/0026-shared-readers-declare-cross-subject-reads.md)) |
...
| [`shared-readers.md`](shared-readers.md) | Reading data your pipeline does **not** own: the `readers/<subject>.py` Shared Readers that declare a cross-subject read once, how to add one (medallion-backed, backed by nothing, or a store over a family such as the review platform's **Question Bank** exports — including when a second grain in one artifact earns a second Reader, when "all of them at once" earns a third, and where a sweep silently double counts), the guardrails G1–G6, and why a Shared Reader is a pass-through and never a shaping layer. |
