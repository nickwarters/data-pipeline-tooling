...
| [`data-dictionary-reviewer-activity.md`](data-dictionary-reviewer-activity.md) | The Monday-Friday, freshness-guarded `reviewer_activity_daily` aggregate and its sparse per-Reviewer `my-stats/{account}.txt` Report Feed, reduced from Sync's `case_current`. |
