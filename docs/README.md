...
| **`Pipeline`** (builder) | The deferred fluent builder: `Pipeline(name, reader).with_processor(…).write_to(writer)`. Call `.describe()` to inspect the planned reader/validators/processors/checkpoints/governance/writer without executing, then `.run(context=…)` to run fail-fast and atomic with RunLog observability. ([ADR-0003](adr/0003-deferred-fluent-builder-composition-model.md)) |
