...
| [`adr/0030-source-checkpoints-are-one-generic-store.md`](adr/0030-source-checkpoints-are-one-generic-store.md) | Decision: where a polled source got to is one generic `SourceCheckpointStore` for every source, keyed by `kind` + stable `key`; the window rule is a pure function handed the position; a ledger of keys already acted on is a different shape and is not folded in. Supersedes the SharePoint-only store, with a carry-over of its file. |
