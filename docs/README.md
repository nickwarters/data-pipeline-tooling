...
| **Eager steps** (`read` / `transform` / `validate` / `write` / `coerce` / `enforce` / `quarantine` / `step`) | The default authoring model: each does its work when called and returns a `Dataset`, so a pipeline is an ordinary Python function that runs top to bottom and can be stepped through in a debugger. Each emits the same run-log record the equivalent `Pipeline` node does, so replay, the run registry, freshness and `cli status` are unaffected. The deferred `Pipeline` builder remains for graphs that genuinely fan in. → [ADR-0027](adr/0027-eager-steps-are-the-default-authoring-model.md) |
