...
| [`migrations.md`](migrations.md) | SQL migrations as the authority for a medallion database's physical shape: the `migrations/<subject>/<layer>/` layout, the per-database `schema_migrations` ledger whose presence is the opt-in, and the three rules (one transaction per file, an edited migration is an error, line endings are not an edit). |
