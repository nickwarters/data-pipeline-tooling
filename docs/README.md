...
| **`PipelineError` / `format_failure`** | The base of the expected fail-fast failure family (`ValidationError`, `FreshnessError`, `UnknownPipelineError`, `RunAddressError`, `CoercionError`, `ForEachPipelineError`, `TransformError`, `PipelineLoadError` all subclass it) and the pure formatter that renders a caught one as a short, traceback-free block for `stderr`. At a run boundary — the operator CLI, a scaffolded `main()` — `except PipelineError` + `format_failure(exc)` turns a deliberate abort into a clear message; a genuine bug is not a `PipelineError` and keeps its trace — except a crash inside a transform step's transformer, which arrives as a located `TransformError` (category `code`) naming the step, the transformer (a lambda by file, line and source) and the line that raised. Row-level breaches from `enforce` / `SchemaValidator` / `SchemaCoercion` name their rows, by `key=` when given, else by 0-based position. |
...
| [`adr/0030-a-transformer-crash-is-a-located-transform-error.md`](adr/0030-a-transformer-crash-is-a-located-transform-error.md) | Decision: a crash inside a transform step's transformer is re-raised as a `TransformError` (category `code`) naming the step, the transformer (a lambda by file, line and source) and the author's line; amends ADR-0005's "a bug keeps its traceback". |
...
| [`adding-a-feed.md`](adding-a-feed.md) | Every Reader, the stubbed remote SharePoint seams, the SharePoint `Modified` checkpoint (`SharePointCheckpointStore`) that computes the next polling window, and the worked incremental feed that wires the two halves together. |
