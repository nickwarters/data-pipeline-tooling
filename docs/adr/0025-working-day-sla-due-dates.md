...
- **Holidays are a plain, maintained list of ISO dates** (D12). Either representation is
  ...
  provision); the helper takes `holidays` as a parameter, so switching the *source* to a
...
- **Calendar-day (not working-day) SLA** — rejected: the business specified *working*
