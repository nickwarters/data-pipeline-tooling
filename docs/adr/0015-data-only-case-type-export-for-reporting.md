...
To decide whether a question *failed*, a report needs that question's
...
- **Port `computeOutcome` to Python.** This forks business logic across two
  ...
  failure is pure data (`failureCriteria`), and *case-level* verdicts are already
...
- **Point Python at the live Question Definitions SharePoint list** ([ADR-0004]).
  That list holds the *shared* half of a question (text, response type, options),
...
- **One file per Case Type**, co-located with the module (mirrors [ADR-0004]'s
  one-module-per-type model). Published through the *same* review/PR flow as the
...
- **Per question:** `id`, `text`, `category`, `responseType`, `options`,
  ...
  *taken* lives on the Answer), and Case-Type operational config
...
- **Failure is derived, not stored, by the report.** Reports replicate
  ...
  `multi-choice` it is array-*includes*. Reports branch on `responseType`.
- **Latest-export semantics for v1.** Python always reads the current
  ...
  uses *current* `failureCriteria` against historical Answers, so historical
  ...
  point-in-time-stable *case verdicts*, reports use the frozen
...
> per-question failure reporting this ADR enables works today; *case-level* verdict
