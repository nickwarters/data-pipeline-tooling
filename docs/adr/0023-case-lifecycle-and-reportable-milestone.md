...
those actions are *sent to the Responsible Party*, worked, and only then is the Case
completed.
...
- The button is shown only when the **Issues Section is complete**: every failed
  Answer's *visible required* Issue Capture Fields are filled ([ADR-0017]/[ADR-0020]
