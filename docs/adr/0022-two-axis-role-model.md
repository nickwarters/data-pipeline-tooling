...
### Axis 1 — functional capability (what you can *do*, anywhere)
...
| `JourneyOwner - <type>` | `ownedJourneyCaseTypes[]` | **Elevated frontline** role for one Case Type — sees the Summary of *every* Case of its type and raises Appeals where the Case Type configures it (see [ADR-0027]). **Not** a Case Type Owner. |
...
### Axis 2 — per-Case-Type list access (which Case's list you can *open*)
...
- **Make `Reviewers - <type>` the *only* reviewer signal** (drop standalone
