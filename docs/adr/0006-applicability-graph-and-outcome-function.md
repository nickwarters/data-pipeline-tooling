...
`showWhen` is per-Case-Type rather than per-Question-Definition because Question Definitions are shared; the same question can wire into different follow-ups across Case Types. The schema starts minimal — `{questionId: {in: [...] | equals:... | answered: true}}` plus `$and` / `$or` — and grows by extension only when a real case demands it. Resist evolving it into a Turing-complete DSL.
