...
| `assigned_at` | `AssignedAt` | `datetime` | Yes | — | When the Case was last handed to its Reviewer. | `2026-07-01T09:15:00+00:00` | None | Conditional invariant: allocated rows require a value; unallocated rows carry null. The SharePoint source column is indexed for Action Centre age/order, with no `Created` fallback. |
