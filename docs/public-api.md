...
| `tools.integrations.sharepoint_rest` — `SharePointModifiedReader`, `SharePointSource`, `ModifiedWindow`, `modified_filters`, `METADATA_COLUMNS`, `SharePointFeedError` | The **incremental** SharePoint Reader: the items of one list whose `Modified` falls in a caller-supplied half-open window, stamped with immutable observation metadata (`METADATA_COLUMNS`). Configures the organisational client behind the `SharePointListClient` seam (`fetch_items(list_name, expand_fields, select_fields, filters)`); the client owns auth, transport and paging. Holds no checkpoint, no retry, and no medallion knowledge — see [adding-a-feed.md](adding-a-feed.md#remote-feeds-sharepoint). Note the seam declares the **fetch alone**, because the window is the caller's to supply; a feed that computes its own window also needs the list server's clock, and states that extension locally (`pipelines/sharepoint_cases`' `CaseListClient`, which extends this Protocol with `server_time()` so a missing client is diagnosed as a missing client). |
| `tools.source_checkpoint` — `SourceCheckpointStore`, `Source`, `SourceIdentity`, `Position`, `Instant`, `POSITION_KINDS`, `InstantWindow`, `instant_window` | The other half of that split, for **every** polled source ([ADR-0030](adr/0030-source-checkpoints-are-one-generic-store.md)): the durable **position** per source, keyed by `kind` + stable `key` (a SharePoint list is its GUID — a title is a mutable display name — and its site is credential-free), and the pure rule over it — `instant_window`: `end = source_now - safety_lag`, `start = committed - overlap` (`None` on a first load), and `None` for the whole window when the safe bound has not advanced yet. `position(...)` never writes; `commit(...)` is the **last act of a successful run**, refuses to move backwards, and nothing else advances a position. A position is what the source is measured in (today `Instant`; a new kind is one class plus one `POSITION_KINDS` entry). This is a base directory's **third** category — source *control state* at `<base>/_checkpoints/sources.db`, beside the rows (`tools.store`) and the run metadata (`tools.observability.run_store`) rather than inside either. Note the word: this "checkpoint" is a source position, **not** the mid-graph `.write()` node the `Pipeline` sense means. See [adding-a-feed.md](adding-a-feed.md#sourcecheckpointstorebase_dir--where-the-polling-got-to). |
