...
| `tools.migrations` — `MigrationRunner`, `Migration`, `AppliedMigration`, `MigrationTarget`, `MigrationError`, `discover_targets`, `is_under_migration_control`, `migrations_directory`, `MIGRATIONS_ROOT`, `LEDGER_TABLE` | The other half of "where a feed lands": what *shape* it lands in. Applies the ordered `.sql` files under `migrations/<subject>/<layer>/` to one database, each file committing with its own row in that database's `schema_migrations` ledger. `pending()` reports without writing; an already-applied file whose checksum changed is an error, not a silent skip. `is_under_migration_control(db_path)` is the cheap check for the ledger's presence — the per-database opt-in that lets subjects convert one at a time. `discover_targets()` walks the tree, which is the *only* registry of which databases are under migration control (`python -m cli migrate` is that walk applied). Run metadata (`_registry/runs.db`, `_orchestration/runs.db`) is out of scope and stays self-migrating from `RUN_RECORD_FIELDS`. See [migrations.md](migrations.md). |
