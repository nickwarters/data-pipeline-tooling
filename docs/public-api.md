...
- `framework/_internal/` — cross-cutting helpers with **no** public name:
  ...
  annotation reading both schema adapters derive from), `row_locator` (how a
  row-level breach names its rows — by key, else by position), `source_location`
  (the line of author code an exception came from, for `TransformError` and
  `PipelineLoadError`), and
  `schema_control` (whether a database declares its own shape). The leading underscore
...
- `cli/` (top-level) — the `python -m cli` **entry point**, not an import
  ...
  `orchestrate` / `runs` / `status` / `log` / `ingest-log` commands), dispatched by
...
| `PipelineError`, `ErrorCategory` | The base of the expected, fail-fast failure family — `ValidationError`, `FreshnessError`, `UnknownPipelineError`, `RunAddressError`, `PipelineGraphError`, `CoercionError`, `ForEachPipelineError`, `TransformError`, `PipelineLoadError`, `RunRegistryLockedError` all subclass it. Catch it at a run boundary to handle any deliberate abort with one `except`; a genuine bug is not a `PipelineError` and keeps its traceback — except a crash inside a transform step's transformer, which arrives as a located `TransformError` with the original chained ([ADR-0030](adr/0030-a-transformer-crash-is-a-located-transform-error.md)). Each carries an `ErrorCategory` (`data` / `operational` / `config` / `code`) recorded on the run log for triage; a raw bug has none. |
...
| `run_pipeline`, `load_pipeline`, `LoadedPipeline`, `PipelineRunner`, `RunContext`, `active_context`, `Requirement`, `FreshnessRequirement`, `FreshnessError`, `UnknownPipelineError`, `PipelineLoadError`, `RunRegistryLockedError` | The `run_pipeline` execution core (used by the path-addressed `run` command) + `load_pipeline(path)`, which resolves a `pipelines/<name>` disk path to its runnable `LoadedPipeline` (`name` / `run` / `upstreams`) — the shared rule the `run` command and the `Orchestrator` both address pipelines by; it raises `UnknownPipelineError` (`config`) only when no pipeline module is at the path (or it has no `run`), and `PipelineLoadError` (`code`) when the module is there but raises while importing, naming the error and the pipeline line it came from — plus the thin domain runner and requirement guard. `FreshnessRequirement` is the compatibility adapter for old pipeline-level freshness checks. `active_context(context)` makes a `RunContext` ambient for a block: `run_pipeline` already does it around a pipeline's `run(context)`, so it is only needed where there is no runner — driving a `run(context)` or a single step by hand from a test or a scratch script. The eager steps read the *ambient* context, so a `RunContext(dry_run=True)` passed as an argument and never made active would be ignored and its held-back writes would land. |
