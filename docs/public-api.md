...
| `tools.baseline_ddl` — `Column`, `columns_of_schema`, `columns_of_table`, `render_create_table`, `render_migration`, `sqlite_type_for`, `SQLITE_TYPES` | Renders a table's shape as SQL, from either of the two honest sources: a declared row dataclass, or a live table a real run produced (the only source that covers a gold aggregate, a quarantine reject table or a raw landing table, none of which has a dataclass). The declared-type → SQLite-type mapping is what `pandas.to_sql` creates for the dtypes `SchemaCoercion` produces, so a generated baseline reproduces today's physical shape. Output is deterministic so it can be regenerated and diffed. Used by `scripts/generate_baseline_migrations.py`; see [migrations.md](migrations.md). |
