...
| `DatasetProfile`, `ColumnProfile`, `profile_dataset`, `ProfileDriftCheck`, `ProfileBaseline`, `ProfileError` | Per-column data profiling (#284): the *statistical* sibling of the run log. `profile_dataset(dataset, columns=…, top_n=…)` computes a `DatasetProfile` (null rate / distinct count / min-max / bounded top-N per column) that `Pipeline.profile(...)` records on the run log; `ProfileDriftCheck(baseline, address, …)` compares it against the median over recent profiled runs (`ProfileBaseline.recent_profiles(...)` — `RunRegistry` is the production source), mirroring `VolumeAnomalyValidator`. A `fail`-severity deviation raises `ProfileError` (an `ErrorCategory.DATA` abort). Re-exported here from `tools.observability`. |
...
(The `framework.run` re-exports also include the profiling surface — `DatasetProfile`, `ColumnProfile`, `profile_dataset`, `ProfileDriftCheck`, `ProfileBaseline`, `ProfileError` — listed with `Pipeline` above.)
...
| `tools.observability` — `RunLog`, `RunRegistry`; `profile` module — `DatasetProfile`, `ColumnProfile`, `profile_dataset`, `ProfileDriftCheck`, `ProfileBaseline`, `ProfileError` | The structured-observability seam and its query store, plus the per-column profiling surface (#284) — the statistical sibling that records per-column shape on the run log and trends it via `RunRegistry.recent_profiles(...)`. All also re-exported via `framework.run`. |
