...
| `DeriveDate` | Gives every row a date inside an inclusive window, derived from the row's key columns and a fixed seed — the same case gets the same date on every run, whatever else is in the batch. |
