...
- `cli/` (top-level) — the `python -m cli` **entry point**, not an import
  ...
  `orchestrate` / `runs` / `status` / `log` commands), and `schema` (the
  `schema diff` command, backed by `tools.schema`), dispatched by
