...
| `SchemaCoercion` | The *coerce* half of the schema adapter: casts each declared column the validator's own dtype check would not already accept — `date` / `datetime` / `bool` out of storage, `str` / `int` / `float` out of a reader's inference — or by the field's own `Coerce` marker where it declares one. A reshape, so it lives here, not with the schema check. |
