...
| `Reader`, `Writer`, `Processor`, `Validator`, `DatasetProfiler`, `Severity` | Shared protocols used by framework internals and available for advanced typing. `DatasetProfiler` (`profile(dataset) -> (payload, warnings)`) is the injected port behind `Pipeline.profile` — the framework drives it and records the payload, while the concrete statistical computation lives in the upper `tools` layer (`DataProfiler`). Concrete implementations still live on their task facades. |
...
| Names | What |
|-------|------|
| `Reader`, `DatasetReader`, `CsvReader`, `StrictCsvReader`, `StrictCsvParseError`, `MissingSourceFileError`, `GlobCsvReader`, `ExcelReader`, `SqliteReader` | The `read() -> Dataset` port and its concrete sources (`StrictCsvReader` is the char-by-char RFC 4180 parser; `StrictCsvParseError` is the located error it raises; `MissingSourceFileError` is the operational `PipelineError` — also a `FileNotFoundError` — every CSV reader raises when its source file is not there). (The remote `SharePointReader` lives in `tools.integrations`, not this facade — see below.) |
...
| `Writer`, `CsvWriter`, `ExcelWriter`, `JsonWriter`, `SqliteTruncateReloadWriter`, `AccumulateByRunWriter`, `SqliteUpsertWriter`, `SqliteInsertOrIgnoreWriter`, `SqliteInsertIfAbsentWriter`, `SqliteAppendOnlyWriter`, `QuarantineWriter`, `StdoutWriter`, `MissingTableError`, `MissingColumnError`, `SqliteWriteError` | The `write(dataset)` port and its concrete sinks (`SqliteInsertIfAbsentWriter` is the `InsertIfAbsent` reference-table sink: it inserts new keys only, minting compact integer surrogates, and never modifies an existing row; `SqliteAppendOnlyWriter` is the `AppendOnly` immutable-version sink: unseen keys append, an unchanged key is a no-op, a changed key raises `AppendOnlyConflictError`; `StdoutWriter` is a console sink for *seeing* a result — e.g. an explainer trace — rather than persisting it; `MissingColumnError` and `SqliteWriteError` are the translated write failures — a column no migration declares, and everything else SQLite refuses a write for). (The remote `SharePointWriter` lives in `tools.integrations`, not this facade — see below.) |
...
| `SchemaCoercion` | The *coerce* half of the schema adapter: casts each declared column the validator's own dtype check would not already accept — `date` / `datetime` / `bool` out of storage, `str` / `int` / `float` out of the text a CSV reader lands or the inference a `SqliteReader` / `ExcelReader` still makes. A reshape, so it lives here, not with the schema check. |
| `CoercionError` | Raised by `SchemaCoercion` on an uncastable value. |
| `summarise`, `statistic`, `ratio`, `ratios`, `total`, `fill_dimensions`, `count_by`, `shaped` | The **aggregate helpers** — the plumbing a gold reduction calls around its group-by, spelled once rather than per pipeline: `summarise(values, prefix, quantiles=(0.5, 0.9))` is the mean, each percentile and the maximum of a column keyed `<prefix>_mean` / `_p50` / `_p90` / `_max`, `None` where there was nothing to take; `ratio` / `ratios` divide and are NULL on a zero denominator; `total` is a sum that stays NULL when no row reported a value; `fill_dimensions` / `count_by` fill a NULL dimension with a literal before grouping and count by grain; and `shaped(rows, schema, stamp=..., sort_by=...)` lands the rows in exactly the columns, order and types the schema dataclass declares, so an empty result passes the same `SchemaValidator` a populated one does. Engine-confined: they take the backing frame's own types, and only `shaped` hands back a `Dataset`. |
...
| `tools.targeted_selection` — `TargetedSelection`, `TargetedSelectionConfig` | A Selection processor (`Dataset` in, `Dataset` out) that reduces a pool to per-cell targets under a total cap, makes up a short cell from the level above, and optionally steers a Boolean column towards 50/50 — see [selection.md](selection.md#targeted-selection--per-cell-targets-made-up-from-the-level-above). |
...
| `tools.observability` — `RunLog`, `RunRegistry`, `RunStore`; `record_schema` module — `RUN_RECORD_FIELDS`, `Field`, `ensure_columns`; `profile` module — `DataProfiler`, `DatasetProfile`, `ColumnProfile`, `profile_dataset`, `ProfileDriftCheck`, `ProfileBaseline`, `ProfileError` | The structured-observability seam and its query store (`RunLog` / `RunRegistry` also re-exported via `framework.run`), plus the per-column profiling surface — the statistical sibling that records per-column shape on the run log and trends it via `RunRegistry.recent_profiles(...)`. `DataProfiler` is the concrete `framework.core.DatasetProfiler` the builder's `.profile(...)` drives; the profiling logic lives here in the upper `tools` layer, never imported down into `framework`. `record_schema` declares the run-record field set **once, as data** — the log record, the registry DDL/migration/`INSERT`/decode and the console line all derive from it, and `tools.orchestration`'s decision store reuses `Field` / `ensure_columns` for its own separate contract. `framework` never imports it: the framework knows the `RunLog` protocol, not the record schema. `run_store` owns the **on-disk layout** of a base directory's run metadata (`_runs/`, `_registry/runs.db`, `_orchestration/runs.db`) and the `catch_up()` sweep over it — the counterpart of `tools.store`'s `StoreRegistry`, which owns where the *data* lands; `timestamps` owns the UTC-instant / local-calendar-date rule every freshness comparison and date-bounded query reads. The `timestamps` module also carries the **column-wide instant helpers** a gold reduction buckets by — `instants` (a whole column of stored ISO text as UTC instants, `NaT` where one does not parse), `local_dates` / `local_date_texts` / `local_months` (each instant's *local* calendar date, as a date, `YYYY-MM-DD` or `YYYY-MM`), and `elapsed(start, end, unit=...)` (a non-negative duration in days, hours or seconds, over two instants or two columns). |
| `tools.integrations.remote` — `SharePointReader`, `SharePointWriter` | The SharePoint list Reader and Writer — same `read()` / `write()` ports as the file/SQLite ones, but reaching a remote client that is **stubbed** behind swappable seams (`SharePointFetcher` / `SharePointPusher`) until the on-prem SE client (NTLM/Kerberos/REST) lands. |
| `tools.integrations.sharepoint_rest` — `SharePointModifiedReader`, `SharePointSource`, `modified_filters`, `METADATA_COLUMNS`, `SharePointFeedError` | The **incremental** SharePoint Reader: the items of one list whose `Modified` falls in a caller-supplied half-open window, stamped with immutable observation metadata (`METADATA_COLUMNS`). Configures the organisational client behind the `SharePointListClient` seam (`fetch_items(list_name, expand_fields, select_fields, filters)`); the client owns auth, transport and paging. Holds no checkpoint, no retry, and no medallion knowledge — see [adding-a-feed.md](adding-a-feed.md#remote-feeds-sharepoint). Note the seam declares the **fetch alone**, because the window is the caller's to supply; a feed that computes its own window also needs the list server's clock, and states that extension locally (`pipelines/sharepoint_cases`' `CaseListClient`, which extends this Protocol with `server_time()` so a missing client is diagnosed as a missing client). |
| `tools.source_checkpoint` — `SourceCheckpointStore`, `InstantWindow`, `instant_window` | The other half of that split, for **every** polled source ([ADR-0030](adr/0030-source-checkpoints-are-one-generic-store.md)): the durable **watermark** per source, keyed by a stable string key (a SharePoint list is its GUID — a title is a mutable display name — and its site is credential-free), and the pure rule over it — `instant_window`: `end = source_now - safety_lag`, `start = committed - overlap` (`None` on a first load), and `None` for the whole window when the safe bound has not advanced yet. `watermark(...)` never writes; `commit(...)` is the **last act of a successful run**, refuses to move backwards, and nothing else advances a watermark. This is a base directory's **third** category — source *control state* at `<base>/_checkpoints/sources.db`, beside the rows (`tools.store`) and the run metadata (`tools.observability.run_store`) rather than inside either. Note the word: this "checkpoint" is a source watermark, **not** the mid-graph `.write()` node the `Pipeline` sense means. See [adding-a-feed.md](adding-a-feed.md#sourcecheckpointstorebase_dir--where-the-polling-got-to). |
...
- `tools.integrations.remote` (`SharePointFetcher`, `SharePointPusher`,
  ...
  `SharePointReader` / `SharePointWriter` /
