...
| Import | What |
|--------|------|
| `shared.constants` — `DEV_ROOT`, `PROD_ROOT` | Shared root declarations (`data` and `~/pipelines_prod`). These constants do not read the environment; `tools.environments` applies the per-environment OS-variable overrides and expands a leading `~`. |
