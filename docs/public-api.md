...
| `tools.environments` — `base_dir_for`, `resolve_base_dir`, `activate_environment`, `require_declared_tables`, `known_environments` | Resolve a run's medallion `base_dir` from a named environment (`prod` / `dev`), each rooted at an OS environment variable with a `dev` fallback to `./data`. `base_dir_for(env, override=...)` is the call every entry point makes: it honours an explicit `--base-dir` **and** activates the environment's policy — today the require-declared-tables guard ([ADR 0016](adr/0016-migrations-own-table-structure.md)) — so a supplied path never silently opts a run out of it. `activate_environment` is the policy half alone; `require_declared_tables(env)` reads an environment's flag without activating it. The operational env → path mapping the operator CLI and pipeline `main()`s use; see [operator-cli.md](operator-cli.md). |
...
| `tools.migrations` — `tools.migrations.discovery.discover_migrations`; `tools.migrations.scope` (`Scope`, `MigrationTreeError`, `parse_scope_dir`, `scope_from_label`); `tools.migrations.topology` (`Database`, `resolve_databases`, `compose_for_subject_layer`); `tools.migrations.ledger` (`LEDGER_TABLE`, `ensure_ledger`, `ledger_exists`, `applied_migrations`, `record_applied`); `tools.migrations.runner` (`plan_database`, `apply_database`, `split_sql_statements`, `ChecksumMismatchError`, `MigrationApplyError`) | Getting a real database to a declared `Table`'s shape. Directory path is scope, filename is a globally ordered version + slug, no manifest — `discover_migrations` walks and validates the tree. `tools.migrations.topology` resolves a base directory to its medallion databases the one way this repo supports — one `<subject>/<layer>.db` per subject/layer actually present, plus the fixed `platform/registry` database; each database carries its own `schema_migrations` ledger, and a migration file's statements plus its ledger row apply in one transaction the runner owns — so a file containing its own `BEGIN`/`COMMIT` is refused, and a statement SQLite rejects raises `MigrationApplyError` naming the file, the database and the statement rather than a bare `sqlite3` error. `plan_database` is strictly read-only (it never creates a ledger). Coarser layouts were designed and removed rather than shipped broken — see [migrations.md](migrations.md). Backs `python -m cli migrations make` / `migrate`. |
