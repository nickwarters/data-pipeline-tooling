...
2. **Update Schema**: Modify the Python `CaseType` dataclass for that feed to include the new column or changed type. This updates the hard contract so the `SchemaValidator` expects the new shape, and (since it derives its columns from the same dataclass via `columns_of`) the feed's declared `Table` in its `schema.py`'s `TABLES`.
3. **Migrate the Database**: Table structure is a migration's job, never a Writer's (ADR 0015, ADR 0016) — `AccumulateByRunWriter`/`SqliteUpsertWriter`/every other Writer now require the target table to already exist and refuse with a named `MissingTableError` otherwise. Generate the migration from the updated declaration and apply it, rather than hand-writing SQL:
   ```sh
   python -m cli migrations make --feed <feed>   # writes the next ALTER TABLE ... ADD COLUMN file
   python -m cli migrate --env <env> --subject <feed>
   ```
   Review the generated file before committing it (see `docs/migrations.md`); a `NOT NULL` addition needs a mechanical placeholder default the generator adds automatically, which you should very likely replace with a real backfill value before it ships.
