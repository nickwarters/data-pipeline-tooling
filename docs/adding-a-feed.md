...
- **`gold_builder`** is a passthrough to start — reads silver, writes gold — with
  ...
  grain, refreshed whole on every run.
...
in the third hop (`gold.py`, refreshed whole on every poll — see the
...
statement of `run`, below the raw hop, the silver hop and the gold hop.
...
different places. Every hop — raw, silver and gold — is a bare
...
named for a gold table would make "visibly last" less true than the plain `if`
does.
