...
**Status:** Grilled 2026-06-17 — capture engine + tab skeleton landed; CONTEXT.md
and the affected architecture decisions were updated. Two items parked with
tickets: Remediation tab and Amend Outcome → case-level override. See "To grill
later" below.
**Created:** 2026-06-16
...
Today (CONTEXT.md "Section") the tabs are:
...
- [ ] **`showInSummary` flags** — the per-Section Summary inclusion flags
      (CONTEXT.md) need re-mapping to the new Section set. Which of
      Review/Issues/Remediation feed Summary?

---

## 2. Flexible extra-data capture ⚠️ extends/contradicts today's Remediation Details
...
- [ ] **"For each question" — all Answers, or only _failed_ Answers?** Today's
...
- [ ] **Declared per Case Type, not per Question Definition — confirm.**
      Remediation Details deliberately declare fields per _Case Type_, not per
      Question Definition,
      ...
      (and accept the cross-Case-Type leakage)?
...
- [ ] **Storage shape.** Today `Answer.remediationDetails` is a
      `Record<string,string>` — string-keyed strings. Multi-select / checkbox
      groups need arrays; numbers/dates may want typing. Do we keep `Record<string,string>`
      (serialise everything to string) or widen to `Record<string, string|string[]>`?
- [ ] **One blob or extend Answers?** Remediation Details are stored inline on the
      Answer to avoid a second source of truth. Same here, or a new structure
      given groups?
- [ ] **Validation** — `select`/`radio` are validated against `options` at capture
      time. What validation for number/date/free text? Max length?
- [ ] **Required fields & the completion gate.** Today a `required` field blocks
...
- [ ] **Lifecycle.** Today Remediation Details are _stripped_
...
"Summary"; confirm and pin the deltas.
...
- [ ] **QA override section — what's the "refinement"?** This is the new bit. Against
      CONTEXT.md (Answer Override, Effective Answers, Current Outcome), specify
...
2. [ ] **"For each question"** — capture on all Answers or only failed ones; declared
       per Case Type or per Question Definition (vs today's deliberate choice)?
...
- **Override moving from per-Answer to CASE-LEVEL.** The "Amend Outcome" tab is confirmed
  ...
  rather than per-question/per-**Answer**. ⚠️ This **directly contradicts
