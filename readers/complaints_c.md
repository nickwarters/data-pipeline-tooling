```python
"""Shared Reader over the ``complaints_c`` Case Type's published silver.

The Complaints C ingest is the only thing that writes this dataset. This module
...
_SUBJECT = "complaints_c"
_TABLE = "complaints_c"


class ComplaintsCCasesReader:
    """Every Complaints C Case as currently ingested — one row per ``record_id``."""
