...
def _migration_plan(
    ...
    The raw baseline takes the **source's own column names** when they differ
    from the schema's (the ``--from-feed-file`` case where the header is not a
    clean identifier), because raw lands the source faithfully and the rename to
    canonical names happens at silver.
    """
    declared = _schema_fields(spec)
    raw_pairs = (
        list(zip(spec.columns, spec.inferred))
