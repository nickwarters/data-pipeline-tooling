...
Case, hashed under the Case Type's ``name`` as the namespace to mint the
deterministic ``case_id``. Declaring it here once means the raw -> silver spine
and any gold step you add read identity off the same object, so a Case and its
Detail rows derive the same ``case_id`` without a cross-pipeline join.
