...
test('AttributeMenu: unset renders an Attribute trigger, no chip or clear, popover hidden', () => {
  ...
  assert.ok(
    findByClass(host, 'cora-attribute-trigger'),
    'trigger button rendered'
  );
  assert.equal(
    findByClass(host, 'cora-attribute-chip'),
    null,
    'no chip when unset'
  );
  assert.equal(
  ...
  assert.equal(
    findByClass(host, 'cora-attribute-popover').hidden,
    true,
...
test('AttributeMenu: when set, renders a chip with the displayName and a clear button', () => {
  ...
  const chip = findByClass(host, 'cora-attribute-chip');
...
test('AttributeMenu: clicking the chip opens the popover so the person can be changed', () => {
  ...
  findByClass(host, 'cora-attribute-chip')._fire('click');
  ...
  assert.equal(
    findByClass(host, 'cora-attribute-chip').getAttribute('aria-expanded'),
    'true'
  );
});
...
test('AttributeMenu: clicking the trigger opens the popover and reflects aria-expanded', () => {
  ...
  const trigger = findByClass(host, 'cora-attribute-trigger');
  ...
  assert.equal(
    findByClass(host, 'cora-attribute-trigger').getAttribute('aria-expanded'),
    'true'
  );
});
...
test('AttributeMenu: clicking the trigger again closes the popover', () => {
  ...
  findByClass(host, 'cora-attribute-trigger')._fire('click');
  findByClass(host, 'cora-attribute-trigger')._fire('click');
...
test('AttributeMenu: Escape closes the popover when open', () => {
  const { host } = mount();
  findByClass(host, 'cora-attribute-trigger')._fire('click');
  assert.equal(findByClass(host, 'cora-attribute-popover').hidden, false);
...
test('AttributeMenu: Escape is a no-op when the popover is closed', () => {
  ...
  /** @type {any} */ (globalThis).document._fire('keydown', keyEvent('Escape'));
  assert.equal(findByClass(host, 'cora-attribute-popover').hidden, true);
});
...
test('AttributeMenu: disconnect removes its document listeners', () => {
  ...
  const popover = findByClass(host, 'cora-attribute-popover');
