...
function findNavItem(node, href) {
  if (node.tagName === 'A' && node.className.includes('cr-app-nav-item') && node.href === href) return node;
...
test('cr-app-nav: case type owner sees dashboard, reports, and question bank links', () => {
  ...
  assert.ok(findNavItem(el, '#/question-bank'), 'case type owner should see question bank link');
});
...
test('cr-app-nav: responsible party sees dashboard link', () => {
  ...
  assert.equal(findNavItem(el, '#/question-bank'), null, 'responsible party should not see question bank');
});
...
test('cr-app-nav: visitor with no roles sees no nav links', () => {
  ...
  assert.equal(findNavItem(el, '#/question-bank'), null, 'visitor should see no question bank nav item');
});
...
test('cr-app-nav: active item matches current hash exactly', () => {
  ...
  assert.equal(dash.getAttribute('aria-current'), 'page', 'dashboard link should be active');
  const reports = findNavItem(el, '#/reports');
  assert.ok(reports, 'reports link should exist');
  assert.equal(reports.getAttribute('aria-current'), '', 'reports link should not be active');
});
...
test('cr-app-nav: active item matches sub-route prefix', () => {
  ...
  const reports = findNavItem(el, '#/reports');
  assert.ok(reports, 'reports link should exist');
  assert.equal(reports.getAttribute('aria-current'), 'page', 'reports link should be active on sub-route');
  const dash = findNavItem(el, '#/dashboard');
  assert.equal(dash.getAttribute('aria-current'), '', 'dashboard link should not be active');
});
