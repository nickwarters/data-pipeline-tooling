...
import { captureValue, validateCaptureGroups, findCaptureField } from '../src/evaluators/issue-capture.js';
...
/** @type {CaptureField} */
const RADIO_FIELD = { key: 'repeat', label: 'Repeat?', type: 'radio', options: ['Yes', 'No'] };

test('captureValue: records a text value into Answer.capture', () => {
  const out = captureValue({ value: 'No' }, TEXT_FIELD, 'Agent rushed the call');
...
test('captureValue: rejects a radio value that is not one of the options', () => {
  assert.throws(() => captureValue({ value: 'No' }, RADIO_FIELD, 'Maybe'), /Maybe/);
});
...
test('captureValue: clearing the last value drops capture entirely', () => {
  const out = captureValue({ value: 'No', capture: { rootCause: 'Rushed' } }, TEXT_FIELD, '');
...
test('validateCaptureGroups: passes when all field keys are unique across groups', () => {
  const groups = [
    { key: 'g1', label: 'Group 1', fields: [{ key: 'a', label: 'A', type: /** @type {const} */ ('text') }] },
    { key: 'g2', label: 'Group 2', fields: [{ key: 'b', label: 'B', type: /** @type {const} */ ('text') }] },
  ];
...
test('validateCaptureGroups: throws when a field key is duplicated across groups', () => {
  const groups = [
    { key: 'g1', label: 'Group 1', fields: [{ key: 'dup', label: 'A', type: /** @type {const} */ ('text') }] },
    { key: 'g2', label: 'Group 2', fields: [{ key: 'dup', label: 'B', type: /** @type {const} */ ('text') }] },
  ];
...
test('findCaptureField: returns the field with the given key from any group', () => {
  const groups = [
    { key: 'g1', label: 'G1', fields: [{ key: 'a', label: 'A', type: /** @type {const} */ ('text') }] },
    { key: 'g2', label: 'G2', fields: [{ key: 'b', label: 'B', type: /** @type {const} */ ('select'), options: ['x'] }] },
  ];
