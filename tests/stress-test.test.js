...
test('stress fixture: contains conditional questions that depend on prior answers', () => {
  ...
  assert.ok(
    conditional.length > 50,
    'expected at least 50 conditional questions'
  );
});
...
test('stress fixture: applicability evaluation of 500 questions runs in under 50ms', () => {
  ...
  for (const q of qs) {
    ...
    else if (q.responseType === 'single-choice' && q.options)
      answers[q.id] = { value: q.options[0] };
    else if (q.responseType === 'multi-choice' && q.options)
      answers[q.id] = { value: [q.options[0]] };
  }
  ...
  assert.ok(
    elapsedPerRun < 50,
    `expected <50ms per evaluation, got ${elapsedPerRun.toFixed(2)}ms`
  );
});
...
test('stress fixture: toggling a triggering answer changes applicability set in < 5ms', () => {
  ...
  assert.ok(
    setOn.size !== setOff.size,
    'applicability set should differ between Yes and No'
  );
  assert.ok(
    elapsed < 10,
    `expected <10ms for two evaluations, got ${elapsed.toFixed(2)}ms`
  );
});
