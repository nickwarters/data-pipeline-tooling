...
test('root route: mount appends a cr-home with capabilities from context (no redirect)', () => {
  ...
  const appEl = { appendChild(/** @type {any} */ el) { appended.push(el); return el; } };
  ...
  try {
    ...
    assert.equal(home.capabilities, capabilities, 'capabilities assigned from context');
...
test('root route: unmount clears appEl', () => {
  const origDoc = (/** @type {any} */ (globalThis)).document;
  (/** @type {any} */ (globalThis)).document = {
    createElement() { return /** @type {any} */ ({ setAttribute() {} }); },
    createTreeWalker() { return { nextNode() { return null; } }; },
  };

  let cleared = 0;
  const appEl = { appendChild() {}, replaceChildren() { cleared++; } };
