...
const { CROverrideEditor } = await import('../src/components/cr-override-editor.js');
...
const CATALOGUE = [
  { id: 'q1', text: 'Was the call recorded?', responseType: 'yes-no-na', failureCriteria: 'No', remediationActions: ['Re-record'], deprecated: false },
  { id: 'q2', text: 'Was consent given?', responseType: 'yes-no-na', failureCriteria: 'No', remediationActions: ['Get consent'], deprecated: false },
];
...
test('CROverrideEditor: override access on a Completed Case renders the authoring form', () => {
  ...
  assert.ok(findByClass(el, 'cr-override-question'), 'question picker rendered');
  assert.ok(findByClass(el, 'cr-override-reasoning'), 'reasoning input rendered');
...
test('CROverrideEditor: existing Overrides are listed as history', () => {
  const existing = {
    source: 'qa', author: 'qaX', at: '2026-06-10T00:00:00Z', answerKey: 'q1',
    value: 'No', reasoning: 'Was actually not recorded.',
  };
  const { el } = makeEditor({ access: 'read-only', caseOverrides: { overrides: [existing] } });
...
test('CROverrideEditor: blocks authoring and warns when the author is the original Assigned Reviewer', () => {
  const { el } = makeEditor({ currentUser: { id: 'rev1', displayName: 'Reviewer One' } });
  assert.ok(findByClass(el, 'cr-override-self-review'), 'self-review warning shown');
  assert.equal(findByClass(el, 'cr-override-form'), null, 'no authoring form for the original reviewer');
});
...
test('CROverrideEditor: a pass→fail Override is blocked until the completion gate is satisfied', () => {
  ...
  author(el, { answerKey: 'q1', value: 'No', reasoning: 'Reviewer missed it.' });
...
test('CROverrideEditor: a satisfied pass→fail Override saves the complete replacement set', () => {
  ...
  menu.dispatchEvent({ type: 'cr-attribute-change', detail: { attributedParty: { loginName: 'jdoe', displayName: 'J Doe' } } });
  // Reasoning + submit.
  const r = findByClass(el, 'cr-override-reasoning');
  ...
  assert.deepEqual(saved.attributedParty, { loginName: 'jdoe', displayName: 'J Doe' });
  assert.equal(saved.remediationDetails.rootCause, 'Training gap');
  assert.ok(saved.remediationActions?.length, 'the canned replacement action set is attached');
...
test('CROverrideEditor: a select-type Remediation Detail is captured and saved', () => {
  const { el, queue } = makeEditor({
    remediationFields: [{ key: 'severity', label: 'Severity', type: 'select', options: ['Low', 'High'], required: true }],
  });
  const sel = findByClass(el, 'cr-override-question');
  sel.value = 'q1';
  sel._listeners['change'][0]({ target: sel });
...
test('CROverrideEditor: history renders the raw answerKey and an em dash when the Question/value are unknown', () => {
  const existing = { source: 'qa', author: 'qaX', at: 't', answerKey: 'gone', value: undefined, reasoning: 'why' };
  const { el } = makeEditor({ access: 'read-only', caseOverrides: { overrides: [/** @type {any} */ (existing)] } });
...
test('CROverrideEditor: a multi-choice replacement value renders as a comma-joined history value', () => {
  const existing = { source: 'qa', author: 'qaX', at: 't', answerKey: 'q1', value: ['A', 'B'], reasoning: 'why' };
  const { el } = makeEditor({ access: 'read-only', caseOverrides: { overrides: [/** @type {any} */ (existing)] } });
...
test('CROverrideEditor: attribution selection re-renders with the chosen party retained', () => {
  ...
  assert.deepEqual(after.attributedParty, { loginName: 'jdoe', displayName: 'J Doe' });
});

test('CROverrideEditor: a Case with no Responsible Party offers no quick-pick party on the attribute menu', () => {
  const { el } = makeEditor({ attributeFailures: true, caseOverrides: { responsibleParty: '' } });
  const sel = findByClass(el, 'cr-override-question');
  sel.value = 'q1';
  sel._listeners['change'][0]({ target: sel });
  const valEl = findByClass(el, 'cr-override-value-control');
...
test('CROverrideEditor: a single-choice Question draws its declared options and defaults blank when unanswered', () => {
  ...
  el.catalogue = /** @type {any} */ ([
    { id: 'sc', text: 'Channel?', responseType: 'single-choice', options: ['Phone', 'Email'], failureCriteria: 'Email', deprecated: false },
  ]);
  el.connectedCallback();
  const sel = findByClass(el, 'cr-override-question');
  sel.value = 'sc';
...
test('CROverrideEditor: a choice Question and a select detail with no declared options degrade gracefully', () => {
  ...
  el.catalogue = /** @type {any} */ ([
    { id: 'sc2', text: 'Outcome?', responseType: 'single-choice', failureCriteria: 'Bad', deprecated: false },
  ]);
  el.connectedCallback();
  const sel = findByClass(el, 'cr-override-question');
  sel.value = 'sc2';
