...
const { fetchReviewerTeamCases } = await import('../src/services/reviewer-team-fetcher.js');
...
test('fetchReviewerTeamCases: calls listCases once per eligible case type with assignedReviewerManager filter', async () => {
  ...
  await fetchReviewerTeamCases(
    /** @type {any} */ (client),
    'user-rm',
    ['hello-review', 'product-sale-review'],
  );
  assert.equal(client.calls.length, 2, 'should make one call per case type');
  assert.ok(client.calls.every(c => c.assignedReviewerManager === 'user-rm'), 'all calls should filter by managerId');
  assert.ok(client.calls.some(c => c.caseType === 'hello-review'), 'should query hello-review');
  assert.ok(client.calls.some(c => c.caseType === 'product-sale-review'), 'should query product-sale-review');
});
...
test('fetchReviewerTeamCases: merges results from all case types into one array', async () => {
  ...
  const makeRow = (id, caseType) => ({
    id, caseType, title: 'T', status: 'In-progress', assignedReviewer: 'r', responsibleParty: 'rp',
    answers: {}, conversation: [], notes: '', completedAt: null, etag: 'e',
  });
  const client = makeClient({
    'hello-review': [makeRow('c1', 'hello-review'), makeRow('c2', 'hello-review')],
    'product-sale-review': [makeRow('c3', 'product-sale-review')],
  });
  const result = await fetchReviewerTeamCases(/** @type {any} */ (client), 'user-rm', ['hello-review', 'product-sale-review']);
...
test('fetchReviewerTeamCases: returns empty array when no eligible case types', async () => {
  const client = makeClient();
  const result = await fetchReviewerTeamCases(/** @type {any} */ (client), 'user-rm', []);
