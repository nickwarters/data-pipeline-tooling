...
function makeAction(
  /** @type {string} */ id,
  /** @type {Partial<import('../src/services/command-palette-store.js').PaletteAction>} */ overrides = {}
) {
  return {
    id,
    label: `Action ${id}`,
...
/** @returns {any} */
function makeEl() {
  const c = new CORACommandPalette();
  c.connectedCallback();
  return c;
}

/** Always returns the CURRENT overlay (fresh after each re-render). */
function overlay(/** @type {any} */ c) {
  return c._children[0];
}
...
function pressMain(/** @type {any} */ c, /** @type {string} */ key) {
  ...
  for (const h of input._listeners['keydown'] ?? [])
    h({ key, preventDefault: () => {} });
}

// ─── Behavior 8 ─────────────────────────────────────────────────────────────
test('cora-command-palette: renders no overlay when closed', () => {
  resetStore();
  const c = makeEl();
...
// ─── Behavior 9 ─────────────────────────────────────────────────────────────
test('cora-command-palette: renders overlay and input when isOpen is true', () => {
  resetStore();
  const c = makeEl();
  openPalette();
...
// ─── Behavior 10 ────────────────────────────────────────────────────────────
test('cora-command-palette: query signal update re-renders filtered list', () => {
  resetStore();
...
// ─── Behavior 11 ────────────────────────────────────────────────────────────
test('cora-command-palette: ArrowDown/Up move the selected item', () => {
  resetStore();
  ...
  let items = overlay(c).querySelectorAll('.palette-item');
  assert.equal(
    items.findIndex((/** @type {any} */ i) => i.className.includes('selected')),
    0,
    'first item selected'
  );
...
// ─── Behavior 12 ────────────────────────────────────────────────────────────
test('cora-command-palette: Enter executes selected simple action and closes', () => {
  resetStore();
  let called = false;
  register(
    makeAction('exec1', {
...
// ─── Behavior 13 ────────────────────────────────────────────────────────────
test('cora-command-palette: Escape closes palette and resets query', () => {
  resetStore();
  query.set('something');
  openPalette();
  const c = makeEl();
...
// ─── Behavior 14 ────────────────────────────────────────────────────────────
test('cora-command-palette: Enter on input-type action shows inline arg input', () => {
  resetStore();
  register(
    makeAction('inp1', {
      label: 'Go to Case',
  ...
  assert.ok(
    overlay(c).querySelector('.palette-arg-input'),
    'arg input rendered'
  );
...
// ─── Behavior 15 ────────────────────────────────────────────────────────────
test('cora-command-palette: submitting arg input calls handler with value and closes', () => {
  resetStore();
  ...
  register(
    makeAction('inp2', {
      ...
      kind: 'input',
      input: { placeholder: 'Enter case ref…' },
      handler: (/** @type {string|undefined} */ v) => {
        received = v;
      },
    })
  );
...
// ─── Behavior 16 ────────────────────────────────────────────────────────────
test('cora-command-palette: Enter on options-type action renders sub-list', () => {
  resetStore();
  register(
    makeAction('opt1', {
      label: 'Set Response',
...
// ─── Behavior 17 ────────────────────────────────────────────────────────────
test('cora-command-palette: selecting options item calls handler with value and closes', () => {
  resetStore();
  /** @type {string|undefined} */
  let received;
  register(
...
// ─── Behavior 18 ────────────────────────────────────────────────────────────
test('cora-command-palette: Ctrl+K opens palette via document keydown', () => {
  resetStore();
  const c = makeEl();
...
test('cora-command-palette: disconnectedCallback removes document keydown listener', () => {
  resetStore();
