"""Tests for the Shared Reader over the ``complaints_b`` subject's silver.
...
ROWS = [
    ...
    {"record_id": "B3", "category": "sales", "priority": "medium"},
]


def _seed(base_dir, rows: list[dict]) -> None:
    silver = medallion(StoreRegistry(base_dir), "complaints_b").silver
    dataset = Dataset.from_pandas(pd.DataFrame(rows))
    silver.writer("complaints_b", Refresh()).write(dataset)


def test_reads_what_the_producer_landed_in_silver(tmp_path):
    _seed(tmp_path, ROWS)

    assert rows_of(ComplaintsBCasesReader(tmp_path).read()) == ROWS


def test_describe_and_data_locations_delegate_to_the_reader_underneath(tmp_path):
    _seed(tmp_path, ROWS)
    reader = ComplaintsBCasesReader(tmp_path)

    described = reader.describe()
    assert "complaints_b" in described
    assert str(tmp_path / "complaints_b" / "silver.db") in described

    assert reader.data_locations == []
    reader.read()
    silver_db = (tmp_path / "complaints_b" / "silver.db").as_posix()
    assert reader.data_locations == [
        {"namespace": f"sqlite:{silver_db}", "name": "complaints_b"}
    ]
