...
function makeClient(cases = []) {
  return { async listCases() { return cases.map(c => ({ ...c })); } };
}

/** @param {Partial<CaseRow>} overrides @returns {CaseRow} */
function makeCase(overrides) {
  return {
    id: 'c1', caseType: 'hello-review', title: 'T', status: 'In-progress',
    assignedReviewer: 'r', responsibleParty: 'rp', answers: {}, conversation: [],
    notes: '', completedAt: null, etag: 'e', assignedReviewerManager: 'user-rm',
    ...overrides,
  };
}
...
test('cr-reviewer-team-report: renders 4 KPI tiles with correct counts', async () => {
  ...
  assert.ok(hasText(el, '2'), 'should show count of 2 for completed-30d (includes 7d case)');
});
...
test('cr-reviewer-team-report: renders breakdown table rows per case type', async () => {
  ...
  assert.ok(hasText(el, 'product-sale-review'), 'should include product-sale-review row');
});
