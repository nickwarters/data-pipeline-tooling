...
function fixtures() {
  /** @type {CaseRow} */
  const original = {
    id: 'case-14', caseType: 'hello-review', title: 'Hello Review #14',
  ...
  /** @type {CaseRow} */
  const qaCheck = {
    id: 'qa-case-1', caseType: 'qa-hello-review', title: 'QA Check — #14',
...
function override() {
  return {
    source: 'qa', sourceCaseId: 'qa-case-1', author: 'user-qa', at: '2026-06-12T00:00:00Z',
    answerKey: 'q-welcome', value: 'No',
    remediationActions: [{ id: 'ov-1', text: 'Coach on greeting.', completed: false }],
...
test('QA Check cross-row Override: the write targets the original row, not the QA Check', async () => {
  ...
  const client = new MockSharePointClient({
    cases: [qaCheck, original], questionDefinitions: [], personas: {},
  });
  ...
  assert.equal(writtenOriginal?.overrides?.length, 1, 'the Override landed on the original row');
  assert.equal(writtenOriginal?.overrides?.[0].sourceCaseId, 'qa-case-1', 'provenance points back to the QA Check');

  const writtenQa = await client.getCase('qa-case-1');
  assert.equal(writtenQa?.overrides, undefined, 'the QA Check row is never written');
  assert.equal(writtenQa?.sourceCaseId, 'case-14', 'the Cases schema round-trips SourceCaseId through the client');
});
...
test('QA Check cross-row Override: a 412 conflict reloads the original ETag and retries the write', async () => {
  ...
  assert.equal(saveQueue.status.get(), 'saved', 'the retry after the 412 reload succeeds');
  const writtenOriginal = await client.getCase('case-14');
  assert.equal(writtenOriginal?.overrides?.length, 1, 'the Override is persisted after the reload-retry');
});
