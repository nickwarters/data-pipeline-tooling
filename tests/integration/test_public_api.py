"""The public framework API: five subpackage facades.

A pipeline author depends on ``framework.io`` / ``framework.transform`` /
``framework.validate`` / ``framework.run`` / ``framework.shared`` — the stable
public surface — not on internal modules by accident. These tests exercise that
surface the way an author would: by building and running a real pipeline through
the facades, and by asserting the internal plumbing stays out of reach.
"""
...
PUBLIC_FACADES = {"io", "transform", "validate", "run", "shared"}
...
def test_package_root_exposes_only_public_facade_modules():
    ...
    assert framework.__all__ == ["io", "transform", "validate", "run", "shared"]
