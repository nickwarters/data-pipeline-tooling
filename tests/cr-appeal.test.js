...
test('CRAppeal: read-only access shows no form and a placeholder when there are no Appeals', () => {
  ...
  el.access = 'read-only';
  el.connectedCallback();
  assert.equal(findByClass(el, 'cr-appeal-form'), null, 'no form for a read-only viewer');
...
const FAIL_CATALOGUE = [
  { id: 'q-greet', text: 'Greeted?', responseType: 'yes-no-na', failureCriteria: 'No', deprecated: false },
  { id: 'q-close', text: 'Closed well?', responseType: 'yes-no-na', failureCriteria: 'No', deprecated: false },
];
...
test('CRAppeal: the form offers a citation checkbox only for failed Answers', () => {
  ...
  assert.equal(boxes.length, 1, 'only the failed Answer is offered for citation');
});
...
test('CRAppeal: raising with a rationale enqueues an additive appeals save with state raised', () => {
  ...
  assert.equal(saved[0].rationale, 'The greeting was actually present.', 'rationale is trimmed');
  assert.ok(saved[0].id, 'an id is assigned');
  assert.ok(saved[0].at, 'a timestamp is stamped');
  assert.equal(saved[0].citedAnswerKeys, undefined, 'no citations when none selected');
});

test('CRAppeal: raising appends to existing resolved Appeals (full history kept)', () => {
  const prior = { id: 'a0', appellant: 'u-rp', at: '2026-01-01T00:00:00Z', rationale: 'old', state: 'resolved' };
...
test('CRAppeal: an empty rationale shows a validation error and does not save', () => {
  ...
  assert.equal(findByClass(el, 'cr-appeal-error').hidden, false, 'error is revealed');
});
...
test('CRAppeal: citing disputed failed Answers records their keys on the Appeal', () => {
  ...
  const saved = queue.enqueued[0].value;
  assert.deepEqual(saved[0].citedAnswerKeys, ['q-greet'], 'only the checked Answer is cited');
});

test('CRAppeal: a raised Appeal does not set Answer values (case-level only)', () => {
  const { el, queue } = makeEditable({ answers: { 'q-greet': { value: 'No' } } });
  findByClass(el, 'cr-appeal-rationale').value = 'dispute';
  findByClass(el, 'cr-appeal-submit')._listeners['click'][0]();
  // Only the `appeals` field is ever written — never `answers`.
  assert.deepEqual(queue.enqueued.map((e) => e.field), ['appeals']);
  assert.deepEqual(el.caseRow?.answers, { 'q-greet': { value: 'No' } }, 'frozen Answers untouched');
});
...
test('CRAppeal: after raising, the form is replaced by an open-Appeal note (one open at a time)', () => {
  ...
  findByClass(el, 'cr-appeal-rationale').value = 'dispute';
  findByClass(el, 'cr-appeal-submit')._listeners['click'][0]();
  assert.equal(findByClass(el, 'cr-appeal-form'), null, 'form removed once an Appeal is open');
  assert.ok(findByClass(el, 'cr-appeal-open-note'), 'open-Appeal note shown');
  assert.ok(findByClass(el, 'cr-appeal-item'), 'the raised Appeal appears in the history');
});

test('CRAppeal: an existing open Appeal blocks the form even before raising', () => {
  const open = { id: 'a1', appellant: 'u-rp', at: '2026-06-10T00:00:00Z', rationale: 'pending', state: 'underReview' };
...
test('CRAppeal: an Appeal item renders its state, rationale and cited Answers', () => {
  ...
  el.access = 'read-only';
  el.caseRow = makeCase({
    appeals: [{ id: 'a1', appellant: 'u-rp', at: '2026-06-10T00:00:00Z', rationale: 'wrong outcome', citedAnswerKeys: ['q-greet'], state: 'raised' }],
  });
  el.connectedCallback();
  const item = findByClass(el, 'cr-appeal-item');
  assert.ok(findByClass(item, 'cr-appeal-state').textContent.includes('raised'));
  assert.equal(findByClass(item, 'cr-appeal-item-rationale').textContent, 'wrong outcome');
...
test('CRAppeal: an Appeal item without citations omits the cited-Answers line', () => {
  ...
  el.access = 'read-only';
  el.caseRow = makeCase({
    appeals: [{ id: 'a1', appellant: 'u-rp', at: '2026-06-10T00:00:00Z', rationale: 'no citations', state: 'raised' }],
  });
  el.connectedCallback();
