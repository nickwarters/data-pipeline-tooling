...
test('CRAppeal: resolving without a saveQueue, caseRow or user does not throw', () => {
  ...
  assert.equal(
    appeal.state,
    'resolved',
    'the Appeal is still resolved in memory'
  );
  assert.equal(
    appeal.resolution.resolver,
    '',
    'a missing user resolves to an empty resolver'
  );
});
...
test('CRAppeal: a QA resolver with an open Appeal sees the resolution form, not the raise form', () => {
  ...
  assert.ok(
    findByClass(el, 'cr-appeal-resolution-rationale'),
    'resolver rationale input'
  );
...
test('CRAppeal: Reject records the rationale, resolves the Appeal, and changes nothing else', () => {
  ...
  assert.equal(
    el.caseRow?.overrides,
    undefined,
    'no override produced by a reject'
  );
...
test('CRAppeal: Agree resolves the Appeal and reveals the Override editor stamped with the Appeal link', () => {
  ...
  el.remediationFields = /** @type {any} */ ([
    { key: 'root', label: 'Root cause', type: 'text', required: true },
  ]);
  el.client = /** @type {any} */ ({});
  el.connectedCallback();

  findByClass(el, 'cr-appeal-resolution-rationale').value =
    'The greeting was present.';
...
test('CRAppeal: resolving keeps prior resolved Appeals and only resolves the open one', () => {
  ...
  assert.equal(
    saved[0].resolution.rationale,
    'no',
    'the prior resolution is untouched'
  );
  assert.equal(saved[1].id, 'a1');
  assert.equal(
    saved[1].state,
    'resolved',
    'the open Appeal is the one resolved'
  );
});
...
test('CRAppeal: after an Appeal is resolved, a QA resolver with no open Appeal sees the empty placeholder', () => {
  ...
  assert.equal(
    findByClass(el, 'cr-appeal-resolve'),
    null,
    'no resolution form without an open Appeal'
  );
  assert.ok(
    findByClass(el, 'cr-appeal-item'),
    'the resolved Appeal still appears in history'
  );
});
...
test('CRAppeal: a QA resolver on a Case with no Appeals sees the empty placeholder and no form', () => {
  ...
  assert.equal(
    findByClass(el, 'cr-appeal-resolve'),
    null,
    'no resolution form without an Appeal'
  );
  assert.ok(findByClass(el, 'cr-appeal-empty'), 'placeholder shown');
});
...
test('CRAppeal: an empty (or missing) resolver rationale shows an error and does not resolve', () => {
  ...
  findByClass(el, 'cr-appeal-resolution-rationale').value = /** @type {any} */ (
    null
  );
  findByClass(el, 'cr-appeal-reject')._listeners['click'][0]();
  assert.equal(queue.enqueued.length, 0, 'nothing persisted');
  assert.equal(
    findByClass(el, 'cr-appeal-resolution-error').hidden,
    false,
    'error revealed'
  );
});
