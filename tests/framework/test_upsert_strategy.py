...
from framework.strategy import UpsertStrategy
...
def test_upsert_insert_only_into_empty_target(tmp_path):
    ...
    writer = store.writer("gold", "entities", UpsertStrategy("id"))
    writer.write(_ds({"id": 1, "name": "Alice"}, {"id": 2, "name": "Bob"}))

    result = store.reader("gold", "entities").read().to_pandas()
    assert len(result) == 2
...
def test_upsert_rejects_missing_key_column(tmp_path):
    store = Store(tmp_path)
    writer = store.writer("gold", "entities", UpsertStrategy("nonexistent_col"))
