...
def test_filter_keeps_only_rows_matching_the_predicate():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2', 'c3'], 'score': [10, 5, 20]}))
...
def test_score_writes_a_column_computed_per_row():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2'], 'amount': [100, 50]}))
...
def test_vectorized_filter_matches_row_filter_for_the_same_rule():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2', 'c3'], 'score': [5, 10, 15]}))
    row = Filter((lambda row: (row['score'] >= 10)))(dataset).to_pandas()
    vectorized = VectorizedFilter((lambda frame: (frame['score'] >= 10)))(dataset).to_pandas()
...
def test_vectorized_derive_matches_row_score_for_the_same_rule():
    ...
    vectorized = VectorizedDerive('priority', (lambda frame: (frame['amount'] * 2)))(dataset).to_pandas()
    assert (vectorized.to_dict('records') == row.to_dict('records'))
...
def test_vectorized_processors_call_their_rules_once_per_dataset():
    ...
    assert (result.to_dict('records') == [{'case_ref': 'c2', 'score': 10, 'double_score': 20}, {'case_ref': 'c3', 'score': 15, 'double_score': 30}])
...
def test_sort_orders_rows_by_a_column():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2', 'c3'], 'score': [10, 30, 20]}))
...
def test_sort_resets_the_index_so_position_is_stable():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2'], 'score': [2, 1]}))
...
def test_rename_renames_mapped_columns_and_leaves_the_rest():
    dataset = Dataset.from_pandas(pd.DataFrame({'ref': ['c1'], 'amt': [100], 'note': ['keep']}))
...
def test_join_with_brings_in_the_other_feeds_columns_on_the_key():
    cases = Dataset.from_pandas(pd.DataFrame({'adviser': ['a1', 'a2'], 'case_ref': ['c1', 'c2']}))
    advisers = JoinDependency('advisers', RecordingReader(Dataset.from_pandas(pd.DataFrame({'adviser': ['a1', 'a2'], 'region': ['north', 'south']}))))
...
def test_join_dependency_is_read_once_and_logged_separately(tmp_path):
    ...
    reader = RecordingReader(Dataset.from_pandas(pd.DataFrame({'adviser': ['a1'], 'region': ['north'], 'team': ['A']})))
    ...
    j2 = p.transform(JoinWith(advisers, on='adviser', how='left'), j1, name='j2')
...
def test_join_with_inner_default_drops_unmatched_rows():
    cases = Dataset.from_pandas(pd.DataFrame({'adviser': ['a1', 'a2'], 'case_ref': ['c1', 'c2']}))
    advisers = RecordingReader(Dataset.from_pandas(pd.DataFrame({'adviser': ['a1'], 'region': ['north']})))
    joined = JoinWith(advisers, on='adviser')(cases).to_pandas()
    assert (list(joined['case_ref']) == ['c1'])

def test_anti_join_with_excludes_rows_whose_key_is_in_the_other_dataset():
    cases = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3'], 'amount': [100, 200, 300]}))
    already_seen = Dataset.from_pandas(pd.DataFrame({'case_id': ['c2']}))
    kept = PublicAntiJoinWith(already_seen, on='case_id', name='already-seen')(cases).to_pandas()
    assert (list(kept['case_id']) == ['c1', 'c3'])
    assert (list(kept['amount']) == [100, 300])

def test_anti_join_with_supports_composite_keys():
    cases = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c1', 'c2'], 'run_id': ['r1', 'r2', 'r1'], 'amount': [100, 200, 300]}))
...
def test_anti_join_with_treats_duplicate_other_keys_as_set_membership():
    cases = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3'], 'amount': [10, 20, 30]}))
...
def test_anti_join_dependency_is_read_once_and_logged_separately(tmp_path):
    ...
    aj2 = p.transform(AntiJoinWith(excluded, on='case_id'), aj1, name='aj2')
    result = p.run().to_pandas()
    records = [json.loads(line) for line in log_path.read_text().splitlines()]
    assert (reader.read_count == 1)
    assert (list(result['case_id']) == ['c1', 'c3'])
    assert ([record['step'] for record in records].count('dependency:already-reviewed') == 1)
...
def test_pipeline_filters_one_feed_and_joins_another_feeds_silver(tmp_path):
    ...
    cases.writer('silver', 'cases', Refresh()).write(Dataset.from_pandas(pd.DataFrame({'adviser': ['a1', 'a2', 'a3'], 'case_ref': ['c1', 'c2', 'c3'], 'amount': [100, 5, 50]})))
    advisers.writer('silver', 'advisers', Refresh()).write(Dataset.from_pandas(pd.DataFrame({'adviser': ['a1', 'a3'], 'region': ['north', 'south']})))
    ...
    j = p.transform(JoinWith(reference, on='adviser'), f, name='join')
...
def test_derive_key_stamps_uuid5_for_a_single_column_key():
    dataset = Dataset.from_pandas(pd.DataFrame({'surname': ['SMITH'], 'dob': ['2024-01-15']}))
    result = DeriveKey(into='case_id', namespace=_NS, natural_key=['surname'])(dataset).to_pandas()
...
def test_derive_key_different_namespaces_yield_different_ids():
    ...
    id_ns1 = DeriveKey(into='case_id', namespace=_NS, natural_key=['ref'])(dataset).to_pandas()['case_id'].iloc[0]
    id_ns2 = DeriveKey(into='case_id', namespace=_NS2, natural_key=['ref'])(dataset).to_pandas()['case_id'].iloc[0]
    assert (id_ns1 != id_ns2)

def test_derive_key_multi_column_key_composes_in_declared_order():
    dataset = Dataset.from_pandas(pd.DataFrame({'surname': ['SMITH'], 'dob': ['2024-01-15']}))
    result = DeriveKey(into='case_id', namespace=_NS, natural_key=['surname', 'dob'])(dataset).to_pandas()
    expected = str(_uuid.uuid5(_NS, 'SMITH|2024-01-15'))
    assert (result['case_id'].iloc[0] == expected)

def test_derive_key_multi_column_key_order_matters():
    dataset = Dataset.from_pandas(pd.DataFrame({'surname': ['SMITH'], 'dob': ['2024-01-15']}))
    id_ab = DeriveKey(into='case_id', namespace=_NS, natural_key=['surname', 'dob'])(dataset).to_pandas()['case_id'].iloc[0]
    id_ba = DeriveKey(into='case_id', namespace=_NS, natural_key=['dob', 'surname'])(dataset).to_pandas()['case_id'].iloc[0]
    assert (id_ab != id_ba)
...
def test_latest_per_key_keeps_one_row_per_key_with_maximum_by_value():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['A', 'A', 'B'], 'load_date': ['2024-01-01', '2024-03-01', '2024-02-01'], 'status': ['open', 'closed', 'open']}))
...
def test_latest_per_key_supports_multi_column_keys():
    dataset = Dataset.from_pandas(pd.DataFrame({'type': ['complaint', 'complaint', 'request'], 'ref': ['C1', 'C1', 'C1'], 'load_date': ['2024-01-01', '2024-06-01', '2024-03-01'], 'status': ['open', 'resolved', 'pending']}))
...
def test_latest_per_key_ties_resolve_to_last_row_in_input_order():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['A', 'A'], 'load_date': ['2024-01-01', '2024-01-01'], 'status': ['first', 'second']}))
    result = LatestPerKey(key='case_id', by='load_date')(dataset).to_pandas()
    assert (len(result) == 1)
    assert (result.iloc[0]['status'] == 'second')

def test_latest_per_key_raises_when_key_column_is_missing():
    dataset = Dataset.from_pandas(pd.DataFrame({'load_date': ['2024-01-01'], 'status': ['open']}))
...
def test_latest_per_key_returns_a_dataset_not_a_dataframe():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['A'], 'load_date': ['2024-01-01'], 'status': ['open']}))
    result = LatestPerKey(key='case_id', by='load_date')(dataset)
    assert isinstance(result, Dataset)

def test_select_columns_keeps_only_the_listed_columns():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1'], 'amount': [100], 'product_1': ['widget']}))
...
def test_drop_columns_removes_the_listed_columns_and_keeps_the_rest_in_order():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1'], 'scratch': [1], 'amount': [100]}))
...
def test_unpivot_melts_value_vars_into_one_row_per_value():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2'], 'product_1': ['widget', 'gadget'], 'product_2': ['doodad', None]}))
    result = Unpivot(id_vars=['case_ref'], value_vars=['product_1', 'product_2'], var_name='product_slot', value_name='product_name', drop_empty=False)(dataset).to_pandas()
    assert (len(result) == 4)
    assert (set(result.columns) == {'case_ref', 'product_slot', 'product_name'})

def test_unpivot_drops_null_and_blank_values_by_default():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1'], 'product_1': ['widget'], 'product_2': [None], 'product_3': ['   ']}))
    result = Unpivot(id_vars=['case_ref'], value_vars=['product_1', 'product_2', 'product_3'], var_name='slot', value_name='name')(dataset).to_pandas()
    assert (len(result) == 1)
    assert (result.iloc[0]['name'] == 'widget')

def test_unpivot_returns_a_dataset_not_a_dataframe():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1'], 'product_1': ['widget']}))
    result = Unpivot(id_vars=['case_ref'], value_vars=['product_1'], var_name='slot', value_name='name')(dataset)
    assert isinstance(result, Dataset)

def test_top_n_per_group_n1_keeps_the_single_highest_by_row_per_group():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3', 'c4'], 'adviser': ['a', 'a', 'b', 'b'], 'score': [10, 30, 50, 20]}))
...
def test_top_n_per_group_breaks_score_ties_on_tiebreak_deterministically():
    rows = pd.DataFrame({'case_id': ['c2', 'c1'], 'adviser': ['a', 'a'], 'score': [40, 40]})
    kept = TopNPerGroup(key='adviser', by='score', n=1, tiebreak='case_id')(Dataset.from_pandas(rows)).to_pandas()
    assert (list(kept['case_id']) == ['c1'])

def test_top_n_per_group_n_gt_1_keeps_the_top_n_ranked_per_group():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3', 'c4', 'c5'], 'adviser': ['a', 'a', 'a', 'b', 'b'], 'score': [10, 30, 20, 5, 50]}))
...
def test_top_n_per_group_passes_a_group_smaller_than_n_through_whole():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3'], 'adviser': ['a', 'b', 'b'], 'score': [10, 20, 30]}))
...
def test_top_n_per_group_empty_in_empty_out():
    empty = Dataset.from_pandas(pd.DataFrame({'case_id': [], 'adviser': [], 'score': []}))
    kept = TopNPerGroup(key='adviser', by='score', n=1)(empty)
    assert (len(kept) == 0)

def test_top_n_per_group_supports_a_multi_column_key():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3', 'c4'], 'adviser': ['a', 'a', 'a', 'a'], 'region': ['n', 'n', 's', 's'], 'score': [10, 30, 50, 20]}))
    kept = TopNPerGroup(key=['adviser', 'region'], by='score', n=1)(dataset).to_pandas()
    assert (set(kept['case_id']) == {'c2', 'c3'})

def test_top_n_per_group_ascending_ranks_lowest_first():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3'], 'adviser': ['a', 'a', 'a'], 'score': [10, 30, 20]}))
    kept = TopNPerGroup(key='adviser', by='score', n=1, ascending=True)(dataset).to_pandas()
    assert (list(kept['case_id']) == ['c1'])

def test_top_n_per_group_returns_a_dataset_not_a_dataframe():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1'], 'adviser': ['a'], 'score': [1]}))
...
def test_sample_per_group_keeps_at_most_n_per_group():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3', 'c4', 'c5'], 'adviser': ['a', 'a', 'a', 'b', 'b']}))
...
def test_sample_per_group_is_invariant_to_incoming_row_order():
    rows = {'case_id': ['c1', 'c2', 'c3', 'c4', 'c5'], 'adviser': ['a', 'a', 'a', 'b', 'b']}
    forward = pd.DataFrame(rows)
    shuffled = forward.iloc[[4, 0, 2, 1, 3]].reset_index(drop=True)
    a = SamplePerGroup(key='adviser', n=2, seed=7)(Dataset.from_pandas(forward)).to_pandas()
    b = SamplePerGroup(key='adviser', n=2, seed=7)(Dataset.from_pandas(shuffled)).to_pandas()
...
def test_sample_per_group_passes_a_group_smaller_than_n_through_whole():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3'], 'adviser': ['a', 'b', 'b']}))
...
def test_sample_per_group_supports_a_multi_column_key():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': ['c1', 'c2', 'c3', 'c4', 'c5', 'c6'], 'adviser': ['a', 'a', 'a', 'a', 'a', 'a'], 'region': ['n', 'n', 'n', 's', 's', 's']}))
...
def test_sample_per_group_different_seeds_can_draw_different_samples():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': [f'c{i}' for i in range(20)], 'adviser': (['a'] * 20)}))
...
def test_sample_different_seeds_can_draw_different_samples():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_id': [f'c{i}' for i in range(20)]}))
...
def test_parse_decodes_a_json_column_into_structured_values():
    dataset = Dataset.from_pandas(pd.DataFrame({'case_ref': ['c1', 'c2'], 'payload': ['{"score": 10}', '[1, 2, 3]']}))
...
def test_parse_applies_a_custom_parser_to_several_columns():
    dataset = Dataset.from_pandas(pd.DataFrame({'a': ['1', '2'], 'b': ['3', '4'], 'c': ['x', 'y']}))
