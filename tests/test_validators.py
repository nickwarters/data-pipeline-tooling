...
def _handle(**columns) -> DataHandle:
    return DataHandle.from_pandas(pd.DataFrame(columns or {"id": [1, 2, 3]}))
...
def test_column_validator_raises_when_a_required_column_is_missing():
    ...
    handle = _handle(id=[1, 2], name=["a", "b"])

    with pytest.raises(ValidationError, match="case_ref"):
        ColumnValidator(["id", "case_ref"]).validate(handle)
...
def test_column_validator_passes_when_all_required_columns_present():
    ...
    handle = _handle(id=[1], case_ref=["c1"], extra=["x"])

    ColumnValidator(["id", "case_ref"]).validate(handle)  # does not raise
...
def test_row_count_validator_raises_below_minimum():
    ...
    with pytest.raises(ValidationError, match="below minimum"):
        RowCountValidator(minimum=5).validate(_handle(id=[1, 2, 3]))
...
def test_row_count_validator_raises_above_maximum():
    ...
    with pytest.raises(ValidationError, match="above maximum"):
        RowCountValidator(maximum=2).validate(_handle(id=[1, 2, 3]))
...
def test_row_count_validator_passes_within_inclusive_bounds():
    ...
    RowCountValidator(minimum=3, maximum=3).validate(_handle(id=[1, 2, 3]))
    RowCountValidator(minimum=1).validate(_handle(id=[1, 2, 3]))
