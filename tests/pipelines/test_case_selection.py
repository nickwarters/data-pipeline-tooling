...
def test_capacity_gate_trips_at_ten_in_the_rolling_year():
    nine = [_review(selected_date=date(2026, 1, 1)) for _ in range(9)]
    assert not rules.at_check_capacity(nine, AS_OF)
    assert rules.at_check_capacity(nine + [_review()], AS_OF)
...
def test_type_c_quota_forces_c_when_slots_run_out():
    # 8 checks done, 0 of them C: 2 slots left, 2 C still required -> force C even
    # though a score of 100 is naturally type A.
    history = [_review(selected_date=date(2026, 1, 1), case_type="A") for _ in range(8)]
    assert rules.assign_case_type(100, history, AS_OF) == "C"
    ...
    assert rules.assign_case_type(100, history, AS_OF) == "A"
...
def test_run_assembles_the_selection_pool_from_the_bundled_feeds(tmp_path):
    ...
    # One Case per eligible adviser, highest-risk first.
    ...
    assert [r["case_type"] for r in selection_pool] == ["C", "B", "B", "C"]
    ...
    assert "capacity" in reasons["adv003"]
