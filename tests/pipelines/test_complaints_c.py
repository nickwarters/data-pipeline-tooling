"""Tests for the ``complaints_c`` Case Type ingest.
...
from pipelines.complaints_c.pipeline import NAMESPACE, raw_builder, run, silver_builder
...
def test_raw_builder_gates_source_columns():
    ...
    # Missing 'resolution_days' column
    reader = given_rows([{"record_id": "c1", "department": "finance"}])

    p = raw_builder(reader, writer)

    with pytest.raises(
        ValidationError, match="missing required column.*resolution_days"
    ):
        p.run()
...
def test_silver_builder_quarantines_value_rule_breaches():
    ...
    # R001 is valid (resolution_days=10)
    # R002 breaches the Range rule (resolution_days=500)
    reader = given_rows(
        [
            {
                "record_id": "R001",
    ...
    assert_rows_equal(
        writer,
        [{"record_id": "R001", "department": "finance", "resolution_days": 10}],
        ignoring=["run_id"],
    )
    ...
    assert "value not in [0, 365]" in rejects[0]["failed_rule"]
...
def test_silver_builder_aborts_on_structural_breaches():
    ...
    # Missing 'resolution_days', which violates the schema structurally.
    # Structural breaches still abort and bypass quarantine.
    reader = given_rows([{"record_id": "R001", "department": "finance"}])

    p = silver_builder(reader, writer, reject_writer)

    with pytest.raises(ValidationError, match="missing column 'resolution_days'"):
        p.run()
