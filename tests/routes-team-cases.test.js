...
test('routes-team-cases: mounts cr-team-cases with client, currentUser, eligibleCaseTypes', () => {
  ...
  register(router, /** @type {any} */ ({ client, currentUser, eligibleCaseTypes }));
...
test('routes-team-cases: unmount is a no-op (does not throw)', () => {
  ...
  router._container = /** @type {any} */ ({});
  register(router, /** @type {any} */ ({
    client: {}, currentUser: { id: 'u1' }, eligibleCaseTypes: [],
  }));
...
test('routes-team-cases: passes query string from location hash to element', () => {
  elements.length = 0;
  (/** @type {any} */ (globalThis)).location = { hash: '#/team-cases?manager=me&status=overdue' };
  ...
  assert.ok(el.queryString.includes('status=overdue'), 'should pass status param');
