...
const { fetchTeamCases } = await import('../src/services/team-cases-fetcher.js');
...
function makeClient(casesByType = {}) {
  ...
  return {
    ...
    async listCases(filter) {
      calls.push({ ...filter });
      return (casesByType[filter.caseType ?? ''] ?? []).map(/** @param {CaseRow} c */ c => ({ ...c }));
    },
  };
}
...
const row = (id, caseType) => ({
  id, caseType, title: 'T', status: 'In-progress', assignedReviewer: 'r',
  responsibleParty: 'rp', answers: {}, conversation: [], notes: '', completedAt: null, etag: 'e',
});

/** @returns {import('../src/services/team-cases-params.js').TeamCasesParams} */
const baseParams = () => ({ manager: 'me', role: 'reviewer-manager', caseType: null, status: null, completedSince: null, completedUntil: null });

test('fetchTeamCases: fans out to all eligible case types when caseType is null', async () => {
  const client = makeClient();
  await fetchTeamCases(/** @type {any} */ (client), baseParams(), 'u1', ['hello-review', 'product-sale-review']);
...
test('fetchTeamCases: queries only the specified caseType when set', async () => {
  ...
  await fetchTeamCases(/** @type {any} */ (client), params, 'u1', ['hello-review', 'product-sale-review']);
...
test('fetchTeamCases: passes assignedReviewerManager filter for role=reviewer-manager', async () => {
  const client = makeClient();
  await fetchTeamCases(/** @type {any} */ (client), baseParams(), 'mgr-99', ['hello-review']);
...
test('fetchTeamCases: merges results from multiple case type lists', async () => {
  ...
  const result = await fetchTeamCases(/** @type {any} */ (client), baseParams(), 'u1', ['hello-review', 'product-sale-review']);
...
test('fetchTeamCases: returns empty array and makes no calls when eligibleCaseTypes is empty', async () => {
  const client = makeClient();
  const result = await fetchTeamCases(/** @type {any} */ (client), baseParams(), 'u1', []);
