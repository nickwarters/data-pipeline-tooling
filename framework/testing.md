...
from framework.readers import DatasetReader, Reader
...
class RecordingWriter:
    """A :class:`~framework.writers.Writer` that captures writes in memory.
...
def read_rows(store: Any, layer: Any, table: str) -> list[dict[str, Any]]:
    ...
    Reader for ``(layer, table)`` — a :class:`~framework.store.Store` or anything
...
class RecordingRunLog(RunLog):
    """A :class:`~framework.run_log.RunLog` that captures records in memory.
...
def read_run_log(path: str | os.PathLike[str]) -> list[dict[str, Any]]:
    ...
    :class:`~framework.run_log.RunLog` to disk (like the demos) is asserted by
