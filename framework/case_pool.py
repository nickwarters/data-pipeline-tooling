...
Type's Cases, read from the **ingested silver** layer, and it offers
domain-named retrievals the Selection pipeline calls. The headline retrieval is
the *concept* of fetching **available cases** — the candidate Cases eligible to
enter Selection, defined by business availability criteria (e.g. activity dated
within the last N working days — CONTEXT.md), computed in **Python**, never SQL
(ADR-0002).

It is scoped **per Case Type** (CONTEXT.md): constructed from that type's
:class:`~framework.case_type.CaseType` (for its schema), its
:class:`~framework.store.Store` (to read its silver), and a
:class:`~framework.calendar.WorkingDayCalendar` (the availability arithmetic).
Silver round-trips dates through SQLite as text, so the pool re-coerces the read
...
class CasePool:
    ...
    def fetch_available_cases(
        ...
        *,
        ...
        silver, repairs the round-trip-lossy date column toward the schema's
        types, then narrows to the working-day window in Python (ADR-0002).
        """
        dataset = self._store.reader("silver", self._case_type.name).read()
