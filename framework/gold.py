...
:class:`~framework.builder.Pipeline` whose terminus is the gold writer: each row
...
own: it passes an explicit :class:`~framework.strategy.AccumulateByRun` strategy
...
def silver_to_gold(
    ...
    *,
    ...
    optional run-log sink. Returns the composed :class:`~framework.builder.Pipeline`;
    call ``.run()`` to execute.
    """
